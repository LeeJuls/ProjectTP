...
| [[알파_개발계획]] §2.6②-b (CSV 기획 데이터 규약) | **유지** — 데이터 스키마라 이 문서 범위 밖. 알파_개발계획.md에 그대로 둠. |
| [[알파_개발계획]] §2.6③ (리소스 관리 규약) | **유지** — 접두사·임포트 표준은 이 문서 §D가 인용만. |
...
> **7 vs 8 표기 차이**: [[기획_방향성]] flow 산문은 7종(Options 미포함, ①~⑨ 서술 흐름 요약). Options는 §2.5(다국어 뼈대 — 언어 선택 UI 필요)의 확정 요구가 8번째로 편입한 것 — flow와 이 문서의 불일치가 아니라 두 근거 문서의 병합.
...
| UI 텍스처 | `T_UI_` | `T_UI_Frame_9slice`·`T_UI_Icon_Gold` | 픽셀퍼펙트 3종 세팅(§2.6③ 계승) |
| 레벨 | (기존) | `map_frontend`·`map_battle` | 3D 무대 |
| 액터 | `BP_` | `BP_BattleSpawnPoint` | (참고) |
...
| **WBP_CharacterCard** | 캐릭터 카드(초상·이름키·포지션·스킬3슬롯) | Draft(Roll/Pick/Ban 전 단계)·Result(최종4 변형) | **높음** | 이 문서 |
...
| `CurrentLanguage` | enum/string | 현재 언어(§2.5) | **SaveGame** |
...
> **PvP 서버 권위 주석**: 실 PvP에서 Rolled/Picked/Banned/Result는 **서버가 확정**하고 GameInstance는 그 확정값의 클라 미러다. 클라 UI는 GameInstance를 읽는다(권위·핫픽스 상세 = [[알파_개발계획]] §2.56, 재설계 아님). 알파(서버 없음)는 GameInstance가 유일 소스.

---

## G. 문자열 규칙 + 스타터 키 목록

- **규칙(계승·재확인)**: UI 표시 문자열 **하드코딩 금지.** 전부 `strings.csv`(`Key,ko,ja,en`) 키 → 런타임 `GetLocalizedString(Key)` (빈칸 폴백 ko). 언어 변경 시 `OnLanguageChanged` 브로드캐스트로 열린 위젯 재바인딩([[알파_개발계획]] §2.5).
