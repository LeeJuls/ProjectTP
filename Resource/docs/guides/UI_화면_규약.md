...
> 상위: [[projectTP_허브]] · 기반: [[기획_방향성]](§방향성2 씬·화면 구조·flow) · [[A0_UMG스파이크]](파이프라인 실측) · [[언리얼_MCP_실전노하우]] §19·§21(UmgMcp 도입).
...
[[알파_개발계획]] §2.6①② 본문은 이 문서로 위임하는 짧은 참조로 교체됨(구버전 상세 삭제).
...
## B. 제작 파이프라인 (A0 실측 반영 · 5단계, UmgMcp 도입으로 갱신 · 2026-07-14)

> **핵심 정정(2026-07-11 확정)**: 구 §2.6①의 "3.전사: MCP가 UMG 위젯 구성"은 **틀렸다.** A0 실측([[A0_UMG스파이크]] §7-5·[[언리얼_MCP_실전노하우]] §19): WBP **껍데기 생성/컴파일/저장은 MCP 가능**하나, **`WidgetTree`/`RootWidget`이 `ObjectTools` read/write 전부 차단** → **위젯 배치(자식 Button/TextBlock 트리)는 MCP 불가.**
>
> **★재정정(2026-07-14)**: 위 결론은 **공식 Unreal MCP(`ObjectTools` 등 기본 툴셋) 한정**이었다. 서드파티 **UmgMcp** 플러그인(UE5.8 전용·UMG 특화, MIT) 도입 후 `create_widget`+`set_widget_properties`(`bIsVariable` 포함)로 위젯 배치 파일럿을 실증 완료 — **"위젯 배치는 MCP 불가"가 뒤집혔다.** 아래 B-1·B-2·B-3을 이 도입 기준으로 갱신. 도입 경위·설치 절차·함정(.NET Framework SDK 필요 등)은 [[언리얼_MCP_실전노하우]] §21.
...
| **4. 위젯 배치** | **Claude (MCP, UmgMcp)** | `UmgMcp.create_widget`(계층 생성) + `set_widget_properties`(`bIsVariable` 포함) — 표준 UMG 클래스명(Button/TextBlock/VerticalBox/Image/Border/Overlay/Spacer/HorizontalBox) | ✅ **2026-07-14 파일럿 실증**(`bIsVariable`까지 확인, [[언리얼_MCP_실전노하우]] §21) | **육안 확인만**(노동 0) |
| **5. 배선** | **Claude (MCP)** | 문자열 바인딩(CSV 키)·이벤트 그래프(OnClicked 등)·데이터 바인딩 — `bluecode_*`(UmgMcp) 자동화 후보 | ⚠ **부분 미검증**(B-3, 갱신) | (폴백 시 클릭 소수) |

> **4번 이력**: 2026-07-11~07-13은 "오너 (수동), 애셋당 수 분 ← 유일한 오너 노동"이었다(A0① 공식 MCP 실측 기준). UmgMcp 도입(2026-07-14)으로 위 표 내용으로 대체.

### B-2. 담당 분리 원칙 (오너가 헷갈리지 않게) — 2026-07-14 UmgMcp 반영

```
Claude:  ①디자인 ②스펙 ③골격생성 ④위젯배치(UmgMcp) ⑤배선(부분 자동, B-3)
         └───────────────── 자동(반복 노동) ─────────────────┘
```
- **오너 노동 = 육안 확인(리뷰)만.** 1~4번 전부 Claude+MCP(공식 MCP+UmgMcp 조합). 5번(배선)은 `bluecode_*` 자동화 후보가 열렸으나 **아직 미실증**(B-3) — 실증 전까지는 부분 폴백(오너 클릭 소수) 가능성이 남는다.
- **배치 효율화(권고, 계승)**: 3·4번을 화면 단위로 묶어 처리하면 왕복이 준다 — 8개 화면+컴포넌트 WBP를 한 세션에서 골격생성→배치까지 이어가는 편이 컨텍스트 전환이 적다.
- **순서 강제(계승)**: **`spec.md` 없이 배치하지 않는다.** 승인본과 스펙이 어긋나면 스펙 기준.

> **구 다이어그램(2026-07-11~07-13, UmgMcp 도입 전 — 기록 보존)**:
> ```
> Claude:  ①디자인 ②스펙 ③골격생성 ─────────[오너: ④배치]─────────  ⑤배선  Claude
>          └────── 자동(반복 노동) ──────┘  └ 수동(창의·수 분) ┘  └ 자동(로직) ┘
> ```
> 당시 규약: "오너의 유일한 노동 = 4번(배치), 애셋당 수 분" — A0①(공식 MCP 한정 실측)이 근거였다.

### B-3. 5단계(배선) MCP 경계 — 확정/미검증 (정직 고지, 2026-07-14 갱신)

A0는 위젯을 심을 방법 자체가 없어 **배선을 실측하지 못했다.** UmgMcp 도입(§B-1 4번)으로 위젯은 이제 존재하지만, **배선 자체는 이번 세션에도 실증하지 못했다** — 경계를 분리해 정직하게 고지한다:
...
| **위젯 참조·바인딩** (TextBlock.Text 바인딩, Button `OnClicked` 바인딩) | ⚠ **미검증** — 공식 MCP `add_component_bound_event`가 위젯 델리게이트에 통하는지 A0에서 테스트 불가(버튼이 없었음). UmgMcp `bluecode_*`(connect/apply/compile 등)가 새 후보로 열렸으나 **아직 실호출 미검증** | Claude가 UmgMcp로 (a)위젯 `bIsVariable` 노출(이미 실증 완료, §B-1 4번) (b)`OnClicked` 바인딩 시도 → 실패 시 연결점만 오너 클릭 소수, 로직은 Claude |

> **★2026-07-14 갱신**: `bIsVariable` 노출은 UmgMcp `set_widget_properties`로 이미 자동화 실증 완료([[언리얼_MCP_실전노하우]] §21) — 위 표의 "(a)IsVariable 노출"은 더 이상 오너 몫이 아니다. `bluecode_*`로 (b)`OnClicked` 바인딩·EventGraph 로직까지 자동화될 가능성이 열렸으나, **이 표의 두 행은 실증 전까지 미검증으로 유지**한다.
>
> **후속 실증(예약, 2026-07-14 갱신)**: 다음 UMG 제작 착수 시, Claude가 UmgMcp로 위젯 배치까지 마친 뒤 **`bluecode_*` 1회 프로브**로 이벤트 배선 자동화 여부를 확정한다 — 통하면 배선 전량 MCP, 안 통하면 공식 MCP `add_component_bound_event`를 2차로 시도하고, 그마저 안 되면 기존 폴백(연결점=오너 클릭, 로직=Claude). §J-2·§J-3에 등록.
...
- **값 치환(포지셔널 플레이스홀더)**: 값마다 키를 늘리지 말고 `{0}`,`{1}` 0-index 토큰 템플릿 1개 + 런타임 값 주입(UE `Format Text` 네이티브). 문법·값소스 매핑(자유형)·표시 변환(율→×100은 BP 표시 시점, 데이터는 순수) **전체 규칙의 SSOT = [[데이터_서버_규약]] §1-B** — 여기 중복 서술 안 함. 이미 `UI_DRAFT_PICK_COUNT`("{0}/5")·`UI_BATTLE_COOLDOWN_FORMAT`("CD {0}")가 이 패턴.
...
## J. 오너 결정·입력 필요 (2건 + 해소 1건)

> 임의로 얼버무리지 않고 모음. 나머지(화면 8분류·루트스택·Draft 내부상태·폴더 그룹핑·터치 완화)는 이 문서에서 **결정 완료.** J-3은 UmgMcp 도입(2026-07-14)으로 오너 액션 전제 자체가 사라져 **해소**(기록 보존, 아래 참조).
...
**J-3. 배선(5단계) 미검증 실증 — ✅ 해소(2026-07-14): 오너 에디터 상시 필요 전제 사라짐.**
UmgMcp 도입 전에는 "오너가 첫 Button을 직접 배치한 뒤 프로브"가 전제였으나, 이제 **위젯 배치 자체가 Claude(UmgMcp) 몫**이라 이 전제가 사라졌다(§B-1 4번). 남은 것은 §B-3의 `bluecode_*` 1회 프로브뿐 — 다음 UMG 제작 착수 시 Claude가 단독으로 실행 가능하다(통하면 배선 전량 MCP, 안 통하면 폴백은 연결점=오너 클릭 소수·로직=Claude). **오너 액션은 더 이상 불필요**(에디터를 켜둔 상태 유지만 필요).
