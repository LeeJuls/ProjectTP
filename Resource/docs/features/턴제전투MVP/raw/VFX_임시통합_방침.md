...
## 막타 히트VFX 누락 수정 (2026-07-15, N19)

> 전투완성 F5-2(죽은 유닛 처리) 착수 시 qa-critic이 TC-F5-N19로 예측해둔 회귀를 실제 수정한 건. 출처: [[F5_TC]] N19 · [[F5-2_완료]] 청크2.

### 근본 원인
F5-2 청크2가 `PlayHurtReaction` 앞에 `Branch(bAlive)`(내부적으로 `IfThenElse_3`)를 꽂아 생존/사망을 가르면서, 기존 히트VFX 6단계 체인이 **생존 분기(then, HURT 경로)에만** 남게 됐다. 그 결과 막타(사망 = DYING 분기, `IfThenElse_3.else`)에서는 피격 이펙트가 재생되지 않는다 — 마지막 일격에만 히트 VFX가 빠지는 회귀.

히트VFX 6단계 체인(2단계 배선의 `TakeHit` 분기B와 동일 패턴):
```
SetWorldLocation(EffectQuad, GetActorLocation + MakeVector(0,-80,20))
→ SetMaterial(EffectQuad, 0, HitMID)
→ HitMID.SetScalarParameterValue(TimeOffset, GetGameTimeInSeconds)
→ EffectQuad.SetVisibility(true)
→ RetriggerableDelay(0.55)
→ EffectQuad.SetVisibility(false)
```

### 수정
DYING 분기 진입부(`IfThenElse_3.else → ClickBox`) 사이에 `Sequence`를 삽입해 히트VFX와 DYING 연출을 **병렬**로 돌린다:
- **then_0 = 히트VFX 6단계**: 위 생존 패턴을 복제하되 `EffectQuad`·`HitMID`·위치 계산(`GetActorLocation + MakeVector`)·`GetGameTimeInSeconds`를 **동일 인스턴스로 fan-out 재사용**(신규 데이터 노드 0개). `SetScalarParameterValue`류 3종은 `declaring_class=/Script/Engine.MaterialInstanceDynamic` 명시(§L130 무음 실패 방지).
- **then_1 = 기존 DYING 체인**(Row13/FC5 → Delay0.575 → bFreeze=1): **무변경**.

### 검증 (Director 구조검증)
- 구조: `IfThenElse_3.else(사망) → Sequence[ then_0=히트VFX 6단계 / then_1=DYING 체인 ]`.
- **생존(HURT) 경로 byte-level 무변경** 교차 확인 — 사망 분기에만 VFX를 더했고 생존 분기는 손대지 않음.
- **F5-2·공중부양 수정 무접촉**(같은 BP를 만지지만 해당 서브그래프에 개입 없음).
- 컴파일 0 에러.
- **오너 육안은 아침으로 이월**: 막타에 히트 이펙트가 실제로 뜨는지 시각 확인(방침대로 VFX는 로그 미기록이라 육안 판정이 최종 게이트).
