...
> ### ★F1 종결 — **오탐이었다. 단 그 자리에서 진짜 문제가 나왔다** (D1 실측, 2026-08-13)
> ★**모순이 아니었다**: `GetBattleState` VariableGet의 출력 핀 `type_id`가 직접 조회로 ★**`바이트`(Byte)**다. `list_properties`가 보여준 `"battleState":{"type":"integer"}`는 ★**byte/int32를 포괄하는 JSON Schema 상위 카테고리**였을 뿐이다. `Equal(Byte)`는 원래 맞는 선택이고 ★**화이트리스트는 17 불변**, 형변환 노드 위험 없음.
> ★★**대신 생성 전략이 실패한다**: §3-2가 지시한 *"기존 노드 `type_id` 그대로 생성(제일 안전)"*이 ★**`find_node_types`로 `"Equal(Byte)"`·`"integer>integer"`·`"Equal(Integer)"` 전부 0건**으로 통하지 않는다. **N5·N8 둘 다 `K2Node_PromotableOperator`(UE5 와일드카드 승격 연산자)**라 **승격 후 표시형은 검색되지 않는다.**
> → 대체 확정: `유틸리티|연산자|같음(==)` · `유틸리티|연산자|보다큼(>)` — 상세는 [[FT1-S3_D1_조회결과]]
> ★**교훈**: D1이 **조회 전용**이라 이 실패를 **수술 없이** 발견했다. 단계 분할의 값어치가 첫 단계에서 나왔다.
