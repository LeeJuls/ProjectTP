...
- **진단**: Director가 직접 Bash로 Pillow 스크립트를 실행해 8종 시트 알파채널을 셀 단위로 실측 — 1회 실행으로 원인 확정(이전 조사의 "약 8프레임"이라는 hedge된 결론을 무너뜨림). 이 판단 과정(왜 재위임 대신 직접 진단했는지)은 [[../../reference/언리얼_MCP_실전노하우|실전노하우]] §4-2에 상세 기록.
