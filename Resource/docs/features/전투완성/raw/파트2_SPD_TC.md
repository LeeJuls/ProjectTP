...
stage: 파트2(SPD 턴 순서)
status: PASS
status_note: "핵심 게이트 PASS 2026-07-17 00시경 — 41건(GRAPH 17·PIE 11·회귀 9·데이터 4) 중 **10건 Director 직접 실행·전부 PASS**: GRAPH 5/5(P2-G01·G02·G03·G04·G07, 본 문서 §7이 지목한 \"서브에이전트 보고 불인정\" 5곳 전부)+보너스 2건(P2-G05·G09) + ★★★최강게이트 P2-P01(런타임 spd 8기)+P2-P02·P2-P11. **계획서 ★게이트가 기능에 무감각함(N1, Critical)을 P2-P01로 대체 확인 완료.** 잔여 31건(첫8턴 LOG·신규원장 P2-R01 등)은 Start 클릭 필요해 아침 오너 확인으로 이월. **[2026-07-17 갱신]** P2-R01의 \"23턴 PIE 관측→봉인\" 방식은 무감각 게이트로 폐기, 대체=[[SPD원장_오라클_v1]] 20유닛턴 오라클-diff(오너 S1 게이트). 이월 31건 전부 §8에서 ID별 처분(코어게이트흡수9·S1오라클런흡수10·F9b흡수1·A2회귀스위트9·근거미확인2) 완료 — blanket 통과 없음. 상세: [[파트2_SPD_완료]]"
updated: 2026-07-17
---
...
| ID | 우선 | 검증 내용 | 판정방법 | 기대값 | 근거 |
|---|---|---|---|---|---|
| **P2-R01** | **★★★** | **신규 원장 재수집 = 새 기준선** — 파트1 이월분 흡수 | LOG 원장 수집 | ~~**PIE 1회 관측**(계산 아님)으로 완주 → `Saved/BattleLogs/battle_<시각>.log` 확보 → 이후 기준선으로 봉인.~~ **[정정 2026-07-17]** "관측 1회를 그대로 기준선으로 승격"하는 이 방식은 **폐기**한다 — qa 적대검토 결과 **실패 불가능한 무감각 게이트로 판정**됐다(Critical, 2026-07-17: 관측값 자체가 기준선이 되면 구현이 틀려도 항상 자기 자신과 diff 0으로 일치해 PASS한다). **대체 절차 = 오라클-diff**: [[SPD원장_오라클_v1]](공식+CSV만으로 게임 코드 미접점 독립 유도, §7 CSV 20행)과 라이브 파싱 CSV를 **전 행(공격자·대상·dmg·hpAfter·died) diff**로 대조한다 — 오라클 §6/§8-3 ★H2 규칙("사망턴·승자·최종HP 일치는 PASS 근거 아님") 준수. ⚠**절차**: ①**에디터 재시작으로 `projectTP.log` 롤오버**(또는 **마지막 `turn=1`부터 끝까지만** 취급 — 스크립트가 최신 로그를 **통째로** 긁는데 전투 구분자가 없고 현재 로그엔 오너 테스트분이 이미 섞여 있다) ② PIE 완주(오라클 §8-1 오너 실행 카드 절차 준수) ③ `cd D:\unreal\Resource\docs\scripts && python extract_battle_log.py` ④ 오라클 §7 CSV 20행과 전 행 diff | 계획서 §회귀 파급. ⚠ `extract_battle_log.py`는 **런타임 기록기가 아니라 수동 사후 추출 도구**(BP에 파일쓰기 노드 없음 → "엔진 로그에 구조화 라인 + 사후 추출" 2단 설계). **[[파트1_Start_TC]] P1-R01(**~~23턴~~ **[정정 2026-07-17] 20유닛턴 — [[SPD원장_오라클_v1]]이 정의** diff-0)이 파트2로 이월**됐으므로 본 TC가 그 종착점(현재는 오너 S1 게이트, [[자율작업배치_2026-07-17]] 참고) |
...
> 41건 중 미실행 31건(GRAPH 10·PIE 8·회귀 9·데이터 4)을 ID별로 처분한다. **처분 기준**: 이미 PASS한 게이트가 그 TC의 우려를 실질적으로 해소했으면 **코어 게이트 흡수**, [[SPD원장_오라클_v1]] even-trade 런(S1)이 관측을 자연히 산출하면 **S1 오라클 런 흡수**, 5스킬+광폭화 풀플레이(F9b)라야 관측되면 **F9b 흡수**, 위 어느 것도 해당 없으면 **A2 회귀 스위트 이월**(파트4 선례와 동일 — 향후 캐릭터 코어 확장 시 회귀 방지망), 근거를 찾지 못하면 **근거 미확인 — 이월 유지**로 정직하게 남긴다.

### GRAPH (10건)

| ID | 처분 | 근거 |
|---|---|---|
| P2-G06 | 코어 게이트 흡수 | P2-P01(런타임 spd 8기 실값, PASS)+P2-G07(SelectInt 극성, PASS)이 `SetSpd`가 RowFound 경로에서 정확히 1회 실행돼 옳은 값을 씀을 실질 입증. 정확한 삽입 위치(`SetAtk`/`SetDef`/`DynamicCast_2` 사이)까지는 별도 미확인이나 효과는 이미 실증됨. |
| P2-G08 | S1 오라클 런 흡수 | S1 20턴 런이 실제 턴 순서를 노출한다 — 인코드/디코드가 비대칭이면 [[SPD원장_오라클_v1]] §7 CSV와의 행 diff가 즉시 어긋난다(N4의 OOB 분석과 동일 근거). |
| P2-G10 | A2 회귀 스위트 이월 | "재진입 안전"(재전투 시 누적 방지)은 다회 전투 시나리오가 필요한데 S1(단발)·F9b(단발) 어느 절차에도 재전투가 없다. |
| P2-G11 | A2 회귀 스위트 이월 | "SetSpd 프로젝트 전역 정확히 1곳" 전수조사는 구조 조회 전용이라 S1/F9b의 행동 관측으로는 "다른 곳에 숨은 SetSpd가 없다"까지 확인되지 않는다. |
| P2-G12 | S1 오라클 런 흡수 | 디코드가 `SortedQueue`를 자기참조하면(오배선) 턴 순서가 오라클 기대 패턴에서 즉시 이탈 — S1 diff-0에서 노출된다. |
| P2-G13 | 코어 게이트 흡수 | P2-G01(★★, 이미 PASS)이 정렬→`EnterTurnStart` exec 순서를 확정 — TC 원문 자체가 "G01 성립이 전제"라고 명시하며, 성립 시 정렬의 ActiveUnit 접촉 여부와 무관하게 위험이 봉쇄된다. |
| P2-G14 | 코어 게이트 흡수 | P2-P01·P2-P11이 PIE에서 성공적으로 실행되고 정확한 값을 반환했다는 사실 자체가 `BP_BattleSpawnPoint`·`BP_BattleManager` 컴파일 성공을 전제한다(컴파일 에러가 있으면 PIE 진입 자체가 불가). 명시적 CMP 로그 확인은 없었으나 실행 사실이 강한 정황증거. |
| P2-G15 | A2 회귀 스위트 이월 | 구조적 GRAPH diff는 미실행. S1 런의 데미지 값이 오라클과 일치해야 하므로 `ResolveHit` 본체 무결은 부분적으로 행동 뒷받침되나, `GetOutgoingAtkMult`·`ApplySkillEffects`·`TickStatusesAtTurnEnd`는 even-trade(상태이상 없음)로 노출되지 않는다 — 전체 범위를 커버하지 못해 이월. |
| P2-G16 | 코어 게이트 흡수 | P2-G07(이미 PASS)이 `SelectInt.A`·`보다큼(>).A` 두 소비처를 핀 원문으로 이미 확인 — G16이 요구하는 소비처 목록과 동일 지점. `SetSpdOverride` 완전 부재(0개)까지는 별도 재확인하지 않았으나 위험도 낮음. |
| P2-G17 | A2 회귀 스위트 이월 | `SortKeys`/`SortedQueue`의 타입·스코프(로컬 vs 멤버)는 구조 조회 전용이라 S1/F9b 어느 쪽으로도 관측되지 않는다. |

### PIE (8건)

| ID | 처분 | 근거 |
|---|---|---|
| P2-P03 | S1 오라클 런 흡수 | [[SPD원장_오라클_v1]] §8-1 step2("개전 직후 턴순서 확인")가 정확히 이 값(Start 직후 `TurnQueue`)을 노출한다. |
| P2-P04 | S1 오라클 런 흡수 | S1 20턴 원장의 앞 8행이 정확히 "첫 8턴 ActiveUnit 순서" 데이터다. |
| P2-P05 | S1 오라클 런 흡수 | S1 런도 매턴 `DT_JobStats` 조회를 거치므로 `"JobId lookup failed"` grep은 S1 로그에서 자연 관측된다. |
| P2-P06 | A2 회귀 스위트 이월 | "PIE 재시작 ×3 동일" 검증은 별도 반복 행동이 필요한데 S1·F9b 둘 다 단일 세션 1회 관측만 정의돼 있다. |
| P2-P07 | S1 오라클 런 흡수 | 오라클 §8-2 증거 묶음 항목 5(errors)가 정확히 이 확인("0 errors" 또는 원문)이다. |
| P2-P08 | S1 오라클 런 흡수 | S1 런이 Start 전/직후/전투 중/종료 여러 시점을 자연히 거친다(대체로 커버 — 단 "State 6(End)" 시점 명시는 실행 카드에 없어 일부 시점은 별도 확인 필요할 수 있음). |
| P2-P09 | A2 회귀 스위트 이월 | "첫 전투==재전투 멱등" 검증은 재전투 시나리오가 필요한데 S1·F9b 둘 다 단일 전투다. |
| P2-P10 | 코어 게이트 흡수 | P2-G01(이미 PASS)로 실질 커버 — TC 원문 자체가 "P2-G01 성립이 전제"라고 명시한다. |

### 회귀 (9건)

| ID | 처분 | 근거 |
|---|---|---|
| P2-R01 | S1 오라클 런 흡수 | 본 TC 자체가 2026-07-17 정정 대상(위 §4 표 참고) — "PIE 1회 관측→봉인" 방식은 무감각 게이트로 폐기, 대체 절차가 정확히 S1 오라클 런(오라클-diff)이다. |
| P2-R02 | F9b 흡수 | F8 turn40 부근은 30유닛턴 초과(광폭화 발동조건) 이후라 even-trade(20턴 종료, 광폭화 미발동)로는 도달 불가 — F9b 풀플레이만이 그 구간에 도달한다. |
| P2-R03 | A2 회귀 스위트 이월 | "재전투 ×2 후 큐 길이 불변" 검증은 재전투 시나리오가 필요, S1·F9b 어느 쪽도 없다. |
| P2-R04 | A2 회귀 스위트 이월 | `QA-H4`/`TC-16` 원문 무수정 확인은 구조 diff 전용이라 미실행 — 이후 파트들이 `EnterTurnEnd` 근처를 건드렸다는 기록은 없어 정황상 유지되나 명시적 diff는 없다. |
| P2-R05 | S1 오라클 런 흡수 | S1 런 자체가 Start 버튼 클릭으로 시작되므로 파트1 UI 흐름(Show/Hide, `NotifyAttackButtonClicked`)이 살아있음을 실행 전제로 재확인한다(단 P1-P01~P11 전체 재통과까지는 아님 — 부분 커버). |
| P2-R06 | 코어 게이트 흡수 | P2-P01의 관측 시점(PIE 진입 후, State 0)은 이미 최소 1회의 `InitBattle`→`ResetForBattle` 시퀀스 이후이며, 그 시점에 spd 값이 정확했다는 사실이 `ResetForBattle`이 Spd를 훼손하지 않았음을 실질적으로 뒷받침한다. |
| P2-R07 | **근거 미확인 — 이월 유지** | `plan.md`·`raw/F4_TC.md` 전수 grep(2026-07-17) 결과 "TurnQueue 인덱스 리터럴 금지" 문구 부재 확인 — 스캐폴드 규약 추가가 실행되지 않았다. |
| P2-R08 | S1 오라클 런 흡수 | [[SPD원장_오라클_v1]] §4 원장 자체가 사망 스킵 이벤트(6건)와 스킵 로그를 내장한다(오라클 §4 "스킵 로그" 절) — S1 diff-0가 `TurnCounter` 불변(H-1)을 자연히 검증한다. |
| P2-R09 | **근거 미확인 — 이월 유지** | 3개 하위항목 중 ① `청사진.md` 현재 판에 "큐 = 등록 순서" 텍스트 자체가 부존재(2026-07-17 grep 확인 — 별도 개편으로 소멸했는지 애초 다른 위치였는지 불명) ② `docs/features/전투완성/TC.md` 파일이 이 기능 폴더에 부존재(Glob 확인, 애초 실행 불가능한 대상이었을 가능성) ③ `언리얼_MCP_실전노하우.md` §34 (78)~(86) 전수 확인 결과 N1·N2·N4 신규 등재 없음 — ③은 명시적으로 미실행 확인. 종합해 처분 보류. |

### 데이터 (4건)

| ID | 처분 | 근거 |
|---|---|---|
| P2-D01 | 코어 게이트 흡수 | [[파트2_SPD_완료]] §8 자산표(`BP_BattleSpawnPoint.uasset`·`map_battle_octopath.umap` mtime 갱신 2건)+바이너리 grep("SpdOverride" 신규 등장)이 이미 충족 — 단 OFPA 개별 파일이 아니라 레벨 패키지 통째 저장 방식(§6 "명세 이탈 1건")으로 확인됐다. |
| P2-D02 | A2 회귀 스위트 이월 | "재컴파일 후 재확인"(함정54 방어)은 재컴파일 트리거 자체가 S1/F9b 어느 흐름에도 없어 자연 발생하지 않는다 — 향후 F7b 등에서 이 BP를 다시 편집할 때 재확인 필요. |
| P2-D03 | 코어 게이트 흡수 | P2-P02(이미 PASS, "8기 SpdOverride 전부 상이")가 성립하려면 `SpdOverride`가 Instance Editable이어야 한다(아니면 전 인스턴스가 CDO 기본값으로 동일했을 것) — 타입(Integer)은 §1 레벨표의 정수값들로 정황 확인. |
| P2-D04 | 코어 게이트 흡수 | P2-P11(이미 PASS)이 정확히 이 레벨 원본 배열 값(`[_C_0,_C_4,_C_9,_C_5,_C_2,_C_6,_C_3,_C_7]`)을 PIE 시작 시점에 재확인했다. |

**집계**: 코어 게이트 흡수 9 · S1 오라클 런 흡수 10 · F9b 흡수 1 · A2 회귀 스위트 이월 9 · **근거 미확인 — 이월 유지 2**(P2-R07·P2-R09).
...
- [[features/전투완성/plan|plan]] · [[features/전투완성/청사진|청사진]] (L21 정정 대상 — P2-R09)
