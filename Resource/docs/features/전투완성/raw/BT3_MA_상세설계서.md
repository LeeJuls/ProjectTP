...
- [[../../../plans/자율진행_plan_v2|자율진행_plan_v2]] §2-(가) — "**FT1 = MA 훅(1a) + 자동 시작 토글(1b) + SCF 호출기(1c)**. 셋 다 `BP_BattleManager` 계열 additive 스캐폴드"
...
★**이 규약은 FT1-1b에는 적용되지 않는다.** [[언리얼_MCP_실전노하우]] §6(1049행 인근, "MCP엔 PIE 콘솔 명령 주입 수단 없음 → 임시 스캐폴드(`BeginPlay`+`Delay`→이벤트 호출) 심고 검증 후 제거가 정석")가 권고하는 표준 패턴은 정확히 `BeginPlay`+`Delay`다. FT1-1b는:
...
**목적**: `AU-B3-03`이 지목한 문제 — `ApplyEffectEntry`/`EvaluateCondition`을 **비기본값**(`coeff≠1.0` 등)으로 관통시켜 검증할 실행 수단이 없다. MA(`NotifySkillSelected`/`NotifyUnitClicked`)는 UI 클릭을 흉내낼 뿐 **임의 함수 호출기가 아니다**. [[언리얼_MCP_실전노하우]] §6: "MCP엔 PIE 콘솔 명령 주입 수단 없음 → 임시 스캐폴드가 정석."
...
- **처분**: [[언리얼_MCP_실전노하우]] §6 원칙대로 "검증 후 제거"가 기본이나, FT1 4제약(§7 자율 경계, `Instance Editable bool 기본 false`)이 이미 라이브 경로 무영향을 보장하므로 **⑩b(스캐폴드 처분)에서 최종 판정**한다(F7b v2 §⑩b — "Method A·MOCK은 A2 회귀 스위트가 재사용" 계승).
...
★**2026-08-12 후속 갱신**: 위 판정 이후 착수 순서 자체가 바뀌었다 — 아래 §9-1 참조. 조회는 이제 AT5/AT6-a가 아니라 **AT4-b 세션 말미**(`AT4-b-2`)에 편승한다.

---

## 9-1. ★착수 순서 확정 (2026-08-12, director 6차 + 오너 승인)

기존 자율 경계 제약 4번(*"S1 봉인 확보 후 착수"*)이 폐기되고 **착수 순서가 역전**됐다. 근거: [[../../../plans/자율진행_plan_v2|자율진행_plan_v2]] §5 자율 경계 v2 제약 4번(개정판) · §4-2 의존 그래프 v4.

**무엇이 바뀌었나**: FT1(본 문서가 설계한 MA 훅 `FT1-1a` 포함 검증 하네스 전체)이 **오너 S1 20턴 런보다 먼저** 착수된다. §4-1의 **MA-1a**(결과 동치 게이트 — MA 재생 20행 ↔ [[SPD원장_오라클_v1]] §7 diff 0, 오너 비용 0)가 **수술 전 기준선** 역할을 한다. 오너 S1 런은 FT트랙 착수의 차단 게이트에서 **오너 일괄 확인 세션(0군)의 사후 확인**으로 강등됐다([[../../../오너_대기목록|오너_대기목록]] 0군 #4).

**논거**(director 6차 판정 근거, 오너 승인):
1. 기존 논거(*"`diff≠0` 원인 3분리 불가"*)는 **기준선의 필요성**을 증명할 뿐 **그 기준선이 오너 20턴이어야 함**을 증명하지 않는다
2. **오라클은 손계산·blind·BP 무접근의 완전 독립 대조군**이다(§5 위 표 참조). 수술 **전** MA 20턴 재생 vs 오라클 `diff=0`을 확보하면, 수술 후 `diff≠0`은 곧 수술 버그다 — 3분리 성립
3. ★**BT5 (c) 판정을 뒤집는 게 아니다.** 그때 기각된 (b)는 *"MA 출력을 S1 원장 **대신** 쓰기"*(MA vs MA = 항등, [[BT5_S1봉인수단_판별]])였고, 여기선 심판이 **독립 오라클**이다
4. plan v2가 **이미 같은 패턴을 승인**했다 — MA-1 2축 분해(오너 20턴 → 1턴, 본 문서 §5)
5. 자율경계 제약 1~3(additive 전용·기본 false 토글·롤백 3분리)이 지켜지면, 토글 off의 라이브 경로 무변은 **그래프 정적 검증**으로 보일 수 있다(§9-0 ★1에서 PM이 승인한 MA-2 화이트리스트 패턴과 같은 종류)
6. ★**오히려 낫다** — FT1을 먼저 심으면 오너 S1을 *"하네스가 실재하는 기판"* 위에서 하게 돼 **봉인 가치가 올라간다**

★**안전장치(필수, 위반 시 즉시 정지)**: **수술 전 MA 재생에서 오라클과 `diff≠0`이 나오면 즉시 정지하고 오너 호출한다.** 이 안전장치가 "사후 확인" 강등의 전제조건이다 — 없으면 3분리(§논거 2)가 성립하지 않는다.

**본 문서와의 정합**: 이 순서 변경은 §1~§8의 설계 내용(MA 정의·Delay 0 규약·스캐폴드 명세·타겟 명시 주입·게이트 대응표)을 **하나도 바꾸지 않는다** — 바뀐 것은 "언제 착수하느냐"뿐이다. §9-0 ★3(조회 세션 편승처)은 AT5/AT6-a에서 **AT4-b-2**로 갱신됐다(위 §9-0 "### 3" 절 후속 갱신 참조, [[../../../plans/자율진행_plan_v2|자율진행_plan_v2]] §3 AT트랙 표 `AT4-b-2` 행).
...
[[../../../plans/자율진행_plan_v2|자율진행_plan_v2]] · [[../../스킬연출구조/raw/자율진행_TC|자율진행_TC]] · [[F7b_재개계획_초안]] · [[전진로직_실체_확정]] · [[턴길이_실측확정_2026-08-12]] · [[턴예산_balance판정_2026-08-12]] · [[BP정리_통합명세_2026-08-11]] · [[../../스킬연출구조/raw/D1_4슬롯구조_확정|D1_4슬롯구조_확정]] · [[../../스킬연출구조/raw/FT1-0_TC|FT1-0_TC]] · [[BT5_S1봉인수단_판별]] · [[전투BP_현황도_2026-08-11]] · [[../../../reference/언리얼_MCP_실전노하우|언리얼_MCP_실전노하우]]
