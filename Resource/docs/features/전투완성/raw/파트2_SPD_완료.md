...
**첫 8턴 순서 + 신규 원장**(P2-P04·P2-R01, ~~23턴~~ **[정정 2026-07-17] 20유닛턴 — [[SPD원장_오라클_v1]]이 정의**)은 Start 클릭이 필요한데, 자동화하려면 다음을 전부 통과해야 한다:
...
→ ~~23턴~~ **[정정 2026-07-17] 20유닛턴** 원장 재수집(오라클-diff 방식, [[SPD원장_오라클_v1]])·F8 turn40 기대값 갱신([[파트2_SPD_TC]] P2-R02)은 그다음.
...
실행 안 된 31건 중 실질적으로 새로운 결함을 잡아낼 가능성이 있는 항목은 §7이 이월한 P2-P04·P2-R01(첫 8턴·~~23턴~~ **[정정] 20유닛턴** 원장) 정도이고, 나머지 회귀 TC는 대부분 P2-R01 완주 이후에나 판정 가능한 파생 항목이다. 31건 전체의 ID별 처분(코어 게이트 흡수/F9b 흡수/A2 회귀 스위트 이월/S1 오라클 런 흡수/근거 미확인)은 [[파트2_SPD_TC]] §처분에 정리했다.
...
2. ~~통과 시 verifier가 23유닛턴 신규 원장 수집([[파트2_SPD_TC]] P2-R01 절차대로) → [[야간F9a_풀회귀_완료]] 원장을 대체하는 새 기준선으로 봉인.~~ **[정정 2026-07-17]** 오너가 직접 [[SPD원장_오라클_v1]] §8 실행 카드대로 even-trade 런(20유닛턴, S1 게이트)을 수행하고 오라클 §7 CSV와 전 행 diff 0으로 봉인한다 — "PIE 관측→봉인" 방식은 폐기.
3. 신규 원장 기준으로 F8 turn40 기대값 갱신(P2-R02) + 잔여 GRAPH/PIE/회귀/데이터 TC(31건) 마저 실행 — ID별 처분표는 [[파트2_SPD_TC]] §처분.
4. 전부 통과 시 파트2 게이트를 [[파트2_SPD_TC]] frontmatter에서 "전체 PASS"로 재승격.

---

## 관련
- [[파트2_SPD_TC]] · [[파트1_Start_진행]] · [[features/전투완성/plan|plan]] · [[features/전투완성/청사진|청사진]]
