...
- ~~**게이트**: MA-1★ 재생 원장 ↔ 오너 수동 원장 **전 행 diff 0**(≠0이면 **도구 폐기·⑧ 수동 폴백**) / MA-2 `EnterExecuting`·`ResolveHit`·`EventGraph` 노드수 무변(무접촉 증명)~~ / MA-3 기본 false에서 수동 클릭 경로 무회귀. ※ 도구 검증은 기본공격 패턴 한정 — SELF/조건부 프루브 커버는 ⑧에서 별도.
  ...
  > ★**갱신 2건(2026-08-12, PM)** — 위 MA-1·MA-2 문구는 **구판**이다. 상세: [[BT3_MA_상세설계서]]
  >
  > **MA-1 → 2축 분해**(plan v2가 이미 대체): **MA-1a**(기계 — 오라클 대조) / **MA-1b**(오너 — 수동 원장 1턴). 원문의 *"오너 수동 원장 전 행 diff 0"*은 오너에게 20턴을 손으로 적게 하는 것이라 실행 불가에 가까웠다.
  >
  > ★**MA-2 → 재정의**(원문이 **자기모순**이었다): *"`EventGraph` 노드수 무변"*을 문자 그대로 지키면 **MA 훅 자체가 불가능**한데, 같은 문서 constraint ①이 *"타이밍 필요 시 EventGraph 신규 커스텀 이벤트 경유"*를 허용한다. 취지(*"기존 로직 무접촉"*)로 돌아가 3층으로 나눈다:
  > - `EnterExecuting`/`ResolveHit`: ★**완전 무변**(0 diff) — 원문 그대로, MA는 이 둘을 절대 안 건드린다
  > - `EnterAwaitCommand`/`EnterAwaitTarget`: 말단에 **정확히 1노드**(이벤트 호출), 기존 노드 **0개 변형**
  > - EventGraph: ★**사전 고정 화이트리스트 외 diff 0**
  >
  > ★**화이트리스트를 사전 고정하는 것이 이 재정의의 핵심**이다. *"신설분 제외"*만 쓰면 **"신설분"의 범위를 사후에 넓힐 수 있어** 게이트가 무력해진다. FT1-1a 착수 **전에** 신설 노드의 **이름과 개수를 문서에 못 박고**, 게이트는 그 목록 외 diff 0으로 판정한다(AT6-b의 Holy 임포트 화이트리스트와 같은 패턴).
...
### ⑦ EnterExecuting 재배선 — ~~단독 커밋·단독 게이트~~ → ❌ **2026-08-11 정정**: **⑦-1**(exec 절단·노드 보존)/**⑦-2**(물리 삭제) **2분할로 Director 채택**. 사유: ④a가 `DAMAGE→ResolveHit`을 연결해 ⑦ 이후에도 `ResolveHit` 인라인(쿨다운·BattleLog)이 인터프리터와 함께 발화 — `BattleLog` 캐스트당 2줄(멀티히트면 N줄)이 되어 오라클-diff가 행수부터 FAIL하는 구조였다. 노드를 즉시 물리 삭제하면 아래 "롤백 스톱"이 복구 불가능해지므로, ⑦-1에서 exec만 끊어 고아화(핀 2개 원복으로 완전 복구 가능)하고, ⑦-1 게이트 GREEN 확인 후 ⑦-2에서 물리 삭제한다. 근거: [[BP정리_Director판정_2026-08-11]] 판정1 · [[BP정리_통합명세_2026-08-11]] §4-1 (최대 회귀 표면 성격은 유지)
- `Target`(단일)→`SelectedTargets`(배열) 타입 불일치 → `GetSelectedTargets` 신규 배선 + `Attacker`→`Caster` 확인. `ApplySkillEffects` 캐스트당 정확 1회.
- **게이트**: 오라클-diff 전 행(DAMAGE 최초 유효)★ + **5스킬 회귀 즉시**★(HEAL/STATUS 최초 라이브 지점 — ⑧로 미루지 않음). ⚠ 위 정정에 따라 이 게이트는 **⑦-1** 커밋에서 판정한다(exec 절단 직후).
- **롤백 스톱**: 실패 시 `EnterExecuting→ResolveHit` 원복 — ⑩a 전까지 구컬럼 병존 = 인라인 복구선 생존. ⑦-1 단계에서는 **노드가 보존돼 있어 핀 2개 원복만으로 완전 복구**되고, ⑦-2(물리 삭제) 이후는 세이브포인트 복원만 가능.
...
```
[S1 원장 봉인(오너)] → MA(도구 구축·검증) → ⑥ → ⑤(additive) → ④a→④b → ⑦-1(exec 절단·노드보존)→⑦-2(물리삭제, ★2026-08-11 2분할 정정) → ⑧ → ⑨ → ⑩a → (A2 마감 후 ⑩b)
```
