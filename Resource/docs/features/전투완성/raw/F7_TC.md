...
> 대상: [[features/전투완성/plan|plan]] §F7 · [[상태이상_확정]](δ틱·step4/6 채널·step8.5·ResolveTargetPool·E1~E16·검증훅 SSOT) · [[스탯_전투공식_v1]] §5(정답지)·§8(계약) · [[상태이상_카탈로그_밸런스]] §8-4(TC 시드) · [[F4_TC]](BLOCKER5/HIGH8 계승) · [[F5-2_TC]](TC 스타일·판정법 계승)
...
| 단계 | ID | 조건 → 기대 | 판정 | 상태 |
|---|---|---|---|---|
| F7a | **G01**★ | 메뉴 처리경로(WBP_BattleHUD/SkillMenu)에서 `Equal/NotEqual(Int\|Str\|Byte)`·`Switch`·`Select` 중 한 피연산자가 **구체 SkillId 리터럴**(7종 id/RowName/문자열)인 노드 **0개**. 슬롯→`PendingSkillId` 라우팅은 Category/PrimaryTargetGroup 데이터 구동. 허용예외=명명 폴백멤버 Get | GRAPH | 부분충족 — 리터럴 2곳 제거(`EnterAwaitTarget` "ENEMY1"→`PendingTargetToken`, `EnterExecuting` SkillId 리터럴→`NotifySkillSelected`/Attack박스 세팅), 잔여는 F7b([[야간F7a_스킬메뉴_완료]] §1) |
...
| 단계 | ID | 조건 → 기대 | 판정 | 상태 |
|---|---|---|---|---|
| F7a | **U01**★ | **SELF 즉시실행(MD-3)**: block/SELF PrimaryTargetGroup 슬롯 → NotifySkillSelected → **AwaitTarget 미진입, 즉시 Executing**(SelectedTargets=[caster]). 클릭 대기 hang 0 | GRAPH+PIE | **PASS** — 막기 SELF 즉시시전 E2E 확인(피격 15 반감), 잠복 소프트락(F4부터 미연결이던 카메라 거리퇴화 분기) 검출·수정 후 hang 0 확정. SELF 스킬 카메라 연출 다듬기는 별도로 F7b 이월([[야간F7a_스킬메뉴_완료]] §2⑤·§3·§5) |
| F7a | **U02** | 메뉴 동적행: Category 탭 필터 → 슬롯 라벨 **DT_Strings(NameKey)**(하드코딩0) + 쿨다운 스킬 bEnabled=false + orphan Category 스킬 처리(숨김/폴백) | GRAPH+PIE | 부분충족 — 라벨(`ST_UI Skill.*` 5종)·쿨다운 bEnabled=false는 확인. Category 탭 자체는 F7a 스코프에서 제외(플랫 3행으로 확정, [[F7_스킬아키텍처_확정]] §11-2) → 탭 필터·orphan Category 처리는 F7b 이월([[야간F7a_스킬메뉴_완료]] §1·§3) |
| F7a | **U04**★ | **메뉴 5스킬 라우팅 무회귀**: 메뉴에서 basic/slash/fireball/block/heal 선택 → `PendingSkillId` 세팅 → **기존 인라인 TakeHit** 경유 → 데미지 무변화(basic 30·slash 42·파볼 61·block피해 30·heal +33). F7a는 인터프리터 무접촉 | SCF+LOG | **PASS** — 5스킬 E2E 전부 확인(베기 42+STUN·파이어볼 61+ATK_DOWN+캐스팅모션·막기 SELF즉시+반감·치유 -33 클램프), [[야간F7a_스킬메뉴_완료]] §3 판정표(6차 최종) |
...
| F7a | **E02** | Cost 무소비(MP없음) — Cost 컬럼 미소비, 전 스킬 Cost=0 | DT+GRAPH | 대기 — [[야간F7a_스킬메뉴_완료]]에서 실측 안 됨, 대기 유지 |
...
## 관련
- [[features/전투완성/plan|plan]] §F7 · [[상태이상_확정]](δ·채널·step8.5·E1~E16) · [[스탯_전투공식_v1]] §5·§8 · [[상태이상_카탈로그_밸런스]] §8-4
