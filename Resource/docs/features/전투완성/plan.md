...
- A1 DoD: 핫시트로 스킬 5종 써가며 한 판 승패까지 완주 + 전투로그 전 과정 기록 + 기존 연출(걸음·카메라) 무손상 + F9a 원장 대조 통과 + F9b 오너 확인. **DoD의 "완주·로그·연출 무손상" 조항 자체는 F9a(2026-07-16, [[야간F9a_풀회귀_완료]])로 이미 실증됐다. 실질 잔여는 2건뿐:**
  1. **23유닛턴 원장 재수집** — 파트2(SPD 턴 순서, [[파트2_SPD_완료]])가 `StartBattle()`에서 턴 순서를 SPD 기준(마법사4기부터)으로 재배열해, F9a가 확정한 구원장(고정순서 A1→B1→A2→B2…)을 무효화했다. DoD의 "F9a 원장 대조 통과"는 새 SPD 순서 기준 원장 재수집을 요구한다. **이미 2회 이월**됨 — 1차: 파트1→파트2([[파트1_Start_진행]] §9-6), 2차: 파트2 착수 시점→아침 오너 Start 확인 이후([[파트2_SPD_완료]] §7, 사유: 메뉴 경유 46클릭 자동화 비용+좌표판독 리스크 함정(51)).
  2. **F9b 오너 육안** — 풀플레이 1판 완주(광폭화 유발 포함 30턴+), 미실행.

  **F7b(3층 데이터 인터프리터, 골격 4함수 완료·청크④~⑩ 잔여)는 위 A1 DoD를 막지 않는다** — additive-only 설계(신규 인터프리터가 기존 `Effect*` 컬럼과 병존)라 인라인 F7a 판정이 여전히 작동 중이고, `ResolveHit` 수술은 강제되지 않는다([[F7b_인터프리터_진행상황_인계]] §3-④ 원문: "우리는 additive-only(`Effect*` 병존)라 인라인 F7a 판정이 여전히 작동 중 — 수술 강제성 없음"). F7b는 A1이 아니라 **A2(캐릭터 코어, 1000조합 30원자 모델)의 코드 기반 인프라**다.
