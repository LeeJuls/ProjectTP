...
stage: F
status: WIP
status_note: F0~F9a 게이트 전부 통과, 잔여 F9b(오너 육안 풀플레이)·S1 SPD 오라클런 검증. 원문(3,727자)은 본문 §부록 참고(2026-08-13 frontmatter enum화로 이관).
updated: 2026-07-17
---

# 📋 전투완성(A1) 세부 plan

> 청사진: [[features/전투완성/청사진|청사진]] · 프로세스: [[개발_워크플로우]] · 승인 원본: `C:\Users\user\.claude\plans\humble-purring-glacier.md`(v3, 전 에이전트 8명 리뷰 반영, git 외부) — 이 문서는 그 F0~F9 스펙을 **실행 가능한 절차**로 전개한 것이다.
...
## 전제 조건 (실행 시작 시 오너 액션 — [[features/전투완성/청사진|청사진]] 재확인 표기만, 원문은 그쪽)
1. UE 에디터+MCP 기동.
...
화면 변화가 생기는 단계는 **[개발 → verifier 실증 → 오너 라이브 확인(PIE) → Director 게이트]**로 확장(기존 [개발→verifier→Director]에 오너 라이브 확인 삽입). verifier 통과 후 Director가 "무엇을 클릭하고 무엇을 관찰하면 되는지" 안내 문구를 준비해 오너에게 제시 → 오너 확인 → Director가 다음 단계 진행을 게이트한다. 상세 표는 [[features/전투완성/청사진|청사진]] 참고, 아래는 단계별 체크리스트에 반영.
...
| ID | 조건 → 기대결과 | 판정방법 | 상태 |
|---|---|---|---|
| [F0][TC-F0-01] | H1~H3 재확인 → v1 §8 L224(막기 지속형)·§7-2 L189(splice 금지·큐길이 불변)·§8 step9 L220+§7-6 L194(쿨 세팅/자기턴 감소) 3/3 문면 존재 | 문서 대조 | **하류 커버**(2026-07-17 재판정) — 별도 문서대조 TC로 재실행하지 않음. F4(§8 스켈레톤)~F7(막기 지속형·SELF/ALLY1/ENEMY1 분기 실전배선)이 이 3항목을 그대로 구현·실행해 F4~F7 게이트가 실질 커버(각 게이트 PASS 완료) |
| [F0][TC-F0-02] | 타겟팅 분기표 → ENEMY1/ALLY1/SELF 3분기 동작 각각 정의(ALLY1=자신포함 bAlive 생존자, SELF=AwaitTarget 스킵·즉시커밋·취소창 없음) | 문서 대조 | **하류 커버**(2026-07-17 재판정) — F7(§7-3 NotifySkillSelected 3분기 실전배선)+F0②(ALLY1 확정)가 이 문면을 실행. 개별 TC 재실행 불요, F7 게이트가 실질 커버 |
| [F0][TC-F0-03] | 배정표 실재 → job_stats.csv에 10102000(Hp90/Atk40/Def10)·10202000(Hp80/Atk42/Def6) 존재 + 배정표가 측당 전사2+마법사2 미러 | CSV 조회 | **PASS**(2026-07-17 qa-critic 실측) — `data/job_stats.csv` 직접 조회로 재확인: `10102000`=Hp90/Atk40/Def10(전사2성), `10202000`=Hp80/Atk42/Def6(마법사2성) 정확 일치. F2(TC-F2-06)에서 이미 DT 레벨로도 PASS된 값과 동일 |
| [F0][TC-F0-04]★ | 배정표 슬롯순서 단일화 → plan §F0③(A1/A3=전사)과 balance §8-1(A1/A2=전사) **불일치 해소** — F9a 원장(§3-1)이 쓰는 balance §8-1로 통일 확정 | 문서 대조 | **해소**(2026-07-13 balance §8-1 통일 확정 — 위 F0③ 표 각주 참고. F9a 원장이 이 배정으로 계산돼 있어 이쪽이 권위, 구 표기는 폐기됨) |
| [F0][TC-F0-05] | motions.csv EndBehavior → 17행에 LOOP/REVERT_IDLE/FREEZE_LAST 3값 실재, Row5=REVERT_IDLE·Row13(dying)/Row15(block)=FREEZE_LAST 매핑 | CSV 조회 | **PASS**(2026-07-17 qa-critic 실측) — `data/motions.csv` 직접 조회로 재확인: 17행 중 LOOP 4(Row0~3)·REVERT_IDLE 11(Row4~12,14,16)·FREEZE_LAST 2(Row13,15), Row5=REVERT_IDLE·Row13(dying)/Row15(block)=FREEZE_LAST 전부 정확 일치 |
...
UE 에디터 Content Browser에서 `UserDefinedStruct` 3개 수동 생성(모달 마비 회피 — [[언리얼_MCP_실전노하우]] §19 확정 사실). 제안 폴더: `/Game/Data/Structs`(미확정, 오너 편의 폴더 무방). **CSV 헤더와 struct 필드명은 대소문자까지 정확히 일치해야 함**(네이밍규약§4-B) — `#id_txt`는 `#` 문자를 필드명에 쓸 수 없으므로 아래 제안명 `IdTxt`로 타이핑하고, **이 매핑이 실제로 import 시 값이 채워지는지는 F2 파일럿에서 최초 검증**(미검증 리스크, 아래 §2-3 참고).
...
| [F4][TC-F4-03] | 베기 → 전사=42 / 마법사=46. 스캐폴드로 TakeHit(…,SkillId=31001000) 직접호출(인게임 베기선택은 F7) | 로그 파싱(스캐폴드) | ~~대기~~ → ❌ **2026-08-11 정정**: **PASS**(dmg=42/46, 2026-07-15 오전) — 본 문서 §진행 체크리스트 "F4 게이트: verifier 실증" 항목이 이미 이 판정을 명시하고 있었음(표 갱신 누락). 근거: [[F4_중단_인수인계]] |
...
## 파트1·2·3·4 — Start버튼·SPD 턴순서·막기치유 연출·유닛라벨/힐표시 (F9a 이후 확장, 2026-07-16 착수)

> F9a(풀회귀) 게이트 통과 후, 오너 지시("attack → start로 변경")를 계기로 착수한 후속 4파트. 위 F0~F9 스테이지 코드표 밖의 별도 트랙이며, 원래 **승인 계획서**(`C:\Users\user\.claude\plans\humble-purring-glacier.md`, v1~v3 개정, 전 에이전트 8명 리뷰 반영, git 외부)가 SSOT였다. **★단 이 파일 원문은 파트4 작업 중 덮어쓰기로 소실됐다 — 승인 계획서 원문(humble-purring-glacier.md v1~v3)은 파트4 덮어쓰기로 소실, git 내부 raw/ 완료 문서 + 커밋·태그가 파트1~4의 SSOT다**([[파트4_라벨힐_완료]] §9 SSOT 승격). 진행 서사·오너 확정 결정·검증이 잡은 잠복 결함(Spd=0 레이스·ShowCancel 콜리전 부재·END-02 위증)·Director 자기정정은 [[파트1_Start_진행]] 참고, TC 46건은 [[파트1_Start_TC]] 참고. 파트4 TC 59건은 [[파트4_라벨힐_TC]] 참고.

| 파트 | 내용 | 착수 순서 | 상태(갱신일은 각 셀 참고) |
|---|---|---|---|
| 파트1 | Attack 박스 → Start 버튼(전투 시작을 사람이 선택) | 선행 | **게이트 PASS**(2026-07-16 23시경) — T1 실증+GRAPH 핀 원문 검증+오너 육안 5건 전부 확인. 23턴 diff-0은 파트2 이월([[파트1_Start_진행]] §9) |
| 파트2 | SPD 턴 순서 정렬(`StartBattle()`에서 정렬 — 파트1이 Spd=0 레이스를 구조적으로 제거) | 파트1 게이트 통과 후(2026-07-16 PASS — 23턴 diff-0 재수집은 파트2 착수 작업에 포함, [[파트1_Start_진행]] §9-6) | **핵심 게이트 PASS**(2026-07-17 00시경) — Director 직접검증 GRAPH 5/5+보너스2 · ★★★P2-P01(런타임 spd 8기) 전부 확인(41건 중 10건 실행·전부 PASS). 첫8턴·23턴 신규원장 등 31건은 아침 오너 Start 확인으로 이월([[파트2_SPD_완료]]) |
| 파트3 | 막기·치유 연출(걸음·카메라·잔상·피격리액션이 스킬 Target/Kind를 반영하도록) | 파트1·2와 접점 0(병행 가능) | **핵심 게이트 PASS**(2026-07-17 01시경) — qa-critic 착수전 검출(계획서 5건 중 3건 명세대로면 미작동 확정)+Director 결정 3건 채택 후 구현. Director 직접검증 GRAPH 6/6(P3-G01~G06) PASS+자산 실증+컴파일 0. 38건 중 6건 실행·전부 PASS, 잔여 32건은 아침 오너 육안 확인(막기·치유·회귀 4항목)으로 이월([[파트3_연출_완료]]) |
| 파트4 | 유닛 라벨(`직업 \| 이름`)+치유 제자리 캐스팅+힐 `+N` 초록 표시 | 파트3 완료 후 | **게이트 PASS**(2026-07-17) — 오너 육안 4항목 전부 PASS+Director 직접검증 GRAPH 8건(작업1 라벨 FormatText 체인)+런타임조인 P4C-15(레벨 8기 CharName↔SpdOverride) PASS+컴파일0·디스크저장 실증. TC 59건 중 23건 PASS, 6건 F9b(오너 풀플레이) 이월, 30건 코어게이트 흡수·A2 회귀스위트 이월([[파트4_라벨힐_완료]]) |

**★최강 게이트(파트1) — 파트2로 이월**(오너 결정, 2026-07-16): 원래는 F9a 23유닛턴 원장을 스킬 메뉴 경유로 재수집해 [[야간F9a_풀회귀_완료]] 원장과 diff 0을 확인하는 게이트였으나, 파트2(SPD 정렬)가 곧 턴 순서를 바꿔 무효화될 데이터라 **오너 결정으로 파트2 착수 시 한 번에 재수집**하기로 했다([[파트1_Start_진행]] §9-6). 파트1 자체는 **T1 실증**(메뉴 경유 기본공격 dmg=30=원장값 일치, §9-5) + **GRAPH 편집범위 봉인**([[파트1_Start_TC]] P1-G20)으로 게이트 PASS 판정. 파트2 착수 시 새로 수집하는 원장을 그 단계의 새 기준선으로 봉인한다. → **[갱신, 2026-07-17]** 파트2 착수 시점에도 재수집은 이뤄지지 않았다 — 메뉴 경유 46클릭 자동화 비용과 좌표판독 리스크(함정(51))를 이유로 Director가 **아침 오너 Start 확인 이후**로 한 번 더 이월했다([[파트2_SPD_완료]] §7). 대신 파트2 자체의 핵심 게이트(GRAPH 5/5·★★★P2-P01)는 이날 밤 통과했다.
...
> 판정 원칙: 사실 근거가 있는 항목만 체크한다. 명시적 기록이 없는 항목은 "암묵/미종결"로 남기고 근거를 붙인다(추정 체크 금지).

- [x] F0 문서화(청사진.md+plan.md, 본 문서) — 선결 확정 8건 방향 반영 완료
- [x] F0 TC 작성(qa-critic) — 6건 작성 완료(판정방법 컬럼 포함, 위 §F0 TC표)
- [ ] F0 게이트 통과 (Director 판정) — 개별 TC 6건은 **전부 해소**(2026-07-17 갱신): TC-F0-03·05·06 PASS · TC-F0-04 해소 · TC-F0-01·02 하류 커버(상세 §F0 TC표). 단 F0 게이트 자체의 Director 종합판정 기록은 아직 없음 — 개별 TC 해소가 게이트 자동통과를 의미하지 않으므로 체크 보류
- [x] F1 개발(balance, F0과 병렬) — 완료 2026-07-13([[광폭화_재검증]] — 30유닛턴 확정)
- [x] F1 게이트 통과 (Director 판정 — F8 게이트 겸함) — 2026-07-13, F8 게이트 GO 판정 포함
- [x] F2 개발(motions.csv 완료 / struct 3종 오너 생성 / DT 파일럿 / DT 확장) — 완료 2026-07-14(본 문서 §F2-2-3)
- [x] F2 게이트: verifier 실증 — TC-F2-01~09 전부 PASS(2026-07-14, TC-F2-04는 예상된 무해 FAIL로 판정 완료)
- [x] F2 게이트 통과 (Director 판정) — 2026-07-14
...
- [x] F3 게이트 통과 (Director 판정) — 완료(2026-07-15)
- [x] F4 개발 — 완료 2026-07-15(디스크 저장 확인, `ResolveHit` §8 step0~9+8.5 구현, [[F4_중단_인수인계]])
- [x] F4 게이트: verifier 실증 — 베기검증(TC-F4-03) PASS(dmg=42/46, 2026-07-15 오전, [[F4_중단_인수인계]])
- [ ] F4 게이트: ★오너 라이브 확인(PIE) — 공격하면 HP가 실제로 깎임 — **전용 체크포인트 기록 없음**(F5-1·F5-2 오너 실플레이 중 데미지 적용이 반복 관찰된 것이 간접 근거일 뿐)
  - [ ] 오너 안내 문구 준비(Director) — 미확인
- [ ] F4 게이트 통과 (Director 판정) — **암묵 완료**(명시적 Director 판정 기록 없음. F5가 F4 위에서 정상 진행되고 F9a 23턴 diff-0로 사후 확증됨을 근거로 실질 진행됨)
- [x] F5 개발 — F5-1(사망·승패, 2026-07-15 [[F5-1_완료]])+F5-2(죽은 유닛 처리, 2026-07-15 [[F5-2_완료]]) 완료
- [x] F5 게이트: verifier 실증 — F5-1 PIE 로그 대조 + F5-2 `InitBattle`→`ResetForBattle` 스모크(`Accessed None` 0건)
- [x] F5 게이트: ★오너 라이브 확인(PIE) — 사망(쓰러져 고정)·승패, 한 판이 끝남 — 완료(2026-07-15, 오너 육안 "한쪽편 hp 모두 0 → 멈춤" 확인 [[F5-1_완료]] + 죽은 유닛 처리 오너 실플레이 확인 [[F5-2_완료]])
  - [x] 오너 안내 문구 준비(Director)
- [x] F5 게이트 통과 (Director 판정) — 2026-07-15
- [x] F6 개발 — 완료 2026-07-16 야간([[야간F6_모션데이터구동_완료]])
- [x] F6 게이트: verifier 실증 — 가짜GREEN 2건(FC≥1 가드 반전·SetPendingSkillId 순서결함) 검출·수정 후 무회귀 검증 완료
- [ ] F6 게이트: 오너 라이브 확인(선택 — 회귀 단계, 원할 때만) — 미실행(선택 항목, 비필수)
- [x] F6 게이트 통과 (Director 판정) — 2026-07-16
- [x] F7 개발 — F7a(5스킬+쿨다운 실전배선) 완료 2026-07-16 야간([[야간F7a_스킬메뉴_완료]])
- [x] F7 게이트: verifier 실증 — E2E 6차 검증, 결함 4건+보너스 1건(SELF 소프트락) 검출·수정 후 판정표 전항목 정상(베기42+STUN·파볼61+ATK_DOWN·막기15반감·치유-33 등)
- [ ] F7 게이트: ★오너 라이브 확인(PIE) — 스킬 4종 사용감 — **미실행**(아침 육안으로 이월, [[야간F7a_스킬메뉴_완료]] §5 "아침 육안: 메뉴 사용감·스킬 4종 연출" · [[야간작업_총결산_2026-07-16]] §2)
  - [ ] 오너 안내 문구 준비(Director) — 미확인
- [ ] F7 게이트 통과 (Director 판정) — 개발·verifier 실증(기능 게이트)은 완료했으나 **★필수 오너 라이브 확인이 아직 없어 형식상 미종결**(F7b·F8·F9a는 이 상태에서 이미 비블로킹으로 후속 진행됨)
- [ ] F7b (스테이지 코드표 밖 — F7 내부 서브스테이지, 3층 데이터 인터프리터) — **체크하지 않음, 미완**. 골격 4함수(`EvaluateCondition`·`ResolveEntryTargets`·`ApplyEffectEntry`·`ApplySkillEffects`) + struct 3종·데이터 3층 완료(2026-07-16, [[F7b_struct부트스트랩_완료]]·[[F7b_인터프리터_진행상황_인계]]). **잔여 청크 ④~⑩**(2026-07-16 시점 원안 — ★2026-08-11 기준 [[F7b_재개계획_초안]] v2로 supersede, 아래 괄호가 최신): ④애플리케이터 실연결(최고위험, `ResolveHit`/`ApplyStatus` 결선) ⑤쿨다운·`BattleLog` 캐스트 레벨 ~~이동~~(→**additive만, 인라인 ⑦까지 유지**) ⑥리터럴→`LookupChannel` 제네릭화 ⑦`EnterExecuting` 재배선(인라인→인터프리터 전환점, 최대 회귀 표면 — →**⑦-1 exec 절단·노드보존/⑦-2 물리삭제 2분할, [[BP정리_Director판정_2026-08-11]] 판정1**) ⑧프루브 2종 E2E+기존 5스킬 정답지 회귀 ⑨Category 탭(F7a 이월분) ⑩구 `Effect*` 컬럼 제거+스캐폴드 ~~원복~~(→**⑩a 구컬럼 제거(비가역·스톱)/⑩b 스캐폴드 처분(기본 존치, A2 마감 시 재판정)로 2분할**). **A1 DoD 비차단** — additive-only 설계(`Effect*` 병존)라 인라인 F7a 판정이 여전히 작동 중([[F7b_인터프리터_진행상황_인계]] §3-④), A2(캐릭터 코어) 인프라로 이월. **착수는 S4 게이트 이후** — 청크 재산정·로그 스키마안·Method A 회귀 인프라 세부는 [[F7b_재개계획_초안]] 참고
- [x] F8 개발(F1 게이트 통과 후 착수) — 완료 2026-07-16 야간([[야간F8_광폭화_완료]])
- [x] F8 게이트: verifier 실증 (오너 라이브 확인은 F9b에 묶음, 별도 없음) — PIE 4게이트(turn1/40/50/52) 전부 기대값 일치, 에러 0
- [x] F8 게이트 통과 (Director 판정) — 2026-07-16
- [x] F9a 개발+실증(verifier 기계 원장 대조) — 완료 2026-07-16([[야간F9a_풀회귀_완료]], 23유닛턴 원장 diff 0, 사망 7건 정확) ⚠ **이 원장은 이후 파트2(SPD 턴순서)가 무효화** — 재수집 필요(아래 §검증방침 A1 DoD 잔여 참고)
- [ ] F9b: ★오너 라이브 확인(PIE) — 풀플레이+광폭화+관찰 포인트 — **미실행**
  - [ ] 오너 안내 문구 준비(Director) — 관찰 포인트 3종+광폭화 유발 방법 — 미실행
- [ ] F9 게이트 통과 (Director 최종 판정, A1 DoD 충족 확인) — **미종결**. 잔여 2건: ① 23턴 원장 재수집(파트2가 무효화, 2회 이월) ② F9b 오너 육안(미실행). 상세는 아래 §검증방침
...
- A1 DoD: 핫시트로 스킬 5종 써가며 한 판 승패까지 완주 + 전투로그 전 과정 기록 + 기존 연출(걸음·카메라) 무손상 + F9a 원장 대조 통과 + F9b 오너 확인. **DoD의 "완주·로그·연출 무손상" 조항 자체는 F9a(2026-07-16, [[야간F9a_풀회귀_완료]])로 이미 실증됐다. 실질 잔여는 2건뿐(= [[자율작업배치_2026-07-17]]의 오너 스톱 지점 S1):**
  1. **S1 오라클 런(전 행 diff 0)** — 파트2(SPD 턴 순서, [[파트2_SPD_완료]])가 `StartBattle()`에서 턴 순서를 SPD 기준(마법사4기부터)으로 재배열해, F9a가 확정한 구원장(고정순서 A1→B1→A2→B2…)을 무효화했다. DoD의 "F9a 원장 대조 통과"는 새 SPD 순서 기준 원장 재수집을 요구한다. **[2026-07-17 확정]** 재수집 방식은 "PIE 관측→봉인"(무감각 게이트, qa Critical)을 폐기하고 **오라클-diff**로 교체했다 — 정답지는 [[SPD원장_오라클_v1]](공식+CSV만으로 게임 코드 미접점 독립 유도, **총 20유닛턴**, 3자 독립유도 일치: balance/Director/qa 0-diff). 판정은 라이브 파싱 CSV와 오라클 §7 CSV 20행의 **전 행(공격자·대상·dmg·hpAfter·died) diff 0**(사망턴·승자·최종HP 일치는 PASS 근거 아님, 오라클 §6 ★H2). **이미 2회 이월**됨 — 1차: 파트1→파트2([[파트1_Start_진행]] §9-6), 2차: 파트2 착수 시점→아침 오너 Start 확인 이후([[파트2_SPD_완료]] §7, 사유: 메뉴 경유 46클릭 자동화 비용+좌표판독 리스크 함정(51)) — 현재는 오너 스톱 지점 **S1**로 재편성.
  2. **F9b 오너 육안** — 풀플레이 1판 완주(광폭화 유발 포함 30턴+), 미실행.

  **F7b(3층 데이터 인터프리터, 골격 4함수 완료·청크④~⑩ 잔여)는 위 A1 DoD를 막지 않는다** — additive-only 설계(신규 인터프리터가 기존 `Effect*` 컬럼과 병존)라 인라인 F7a 판정이 여전히 작동 중이고, `ResolveHit` 수술은 강제되지 않는다([[F7b_인터프리터_진행상황_인계]] §3-④ 원문: "우리는 additive-only(`Effect*` 병존)라 인라인 F7a 판정이 여전히 작동 중 — 수술 강제성 없음"). F7b는 A1이 아니라 **A2(캐릭터 코어, 1000조합 30원자 모델)의 코드 기반 인프라**다. 착수는 오너 게이트 **S4**(자율작업배치_2026-07-17 §스톱 지점) 이후 — 세부 재개계획은 [[F7b_재개계획_초안]](청크 재산정·로그 스키마안·Method A 회귀 인프라, 착수 대기).
...
## 부록 — 구 frontmatter `status` 원문 (2026-08-13, 3단계 status enum화로 이관)

> 원본 frontmatter `status` 필드가 3727자로 M1(300자 초과) 규칙 임계를 크게 넘어 본문으로 이관했다(1000자 초과 규칙 적용). frontmatter `status_note`는 요약만 담는다. 전문은 아래(무수정).

F0~F9a 전 스테이지 게이트 통과, 잔여는 F9b(오너 육안)뿐 — F0은 문서화 완료 상태로 TC 6건 개별 상태 전부 해소(2026-07-17 갱신, 상세 §F0 TC표): TC-F0-03·05·06 PASS(2026-07-13/17 확정)·TC-F0-04 해소(2026-07-13 balance §8-1 통일)·TC-F0-01·02는 F4~F7 하류 구현이 커버. 단 **F0 게이트 자체의 Director 종합판정은 개별 TC 해소와 별개로 아직 명시적 기록 없음**(진행 체크리스트 동일). F1(광폭화 재검증) 완료 2026-07-13([[광폭화_재검증]], F8 게이트 GO 겸함). F2(데이터 부트스트랩) 완료 2026-07-14(본 문서 §F2-2-3, DT_Motions/JobStats/Skills/Strings 4종 PASS). F4(TakeHit/ResolveHit §8 코어)는 2026-07-15 베기검증(dmg=42/46 PASS)까지 마쳤으나 **명시적 Director 게이트 판정 기록은 없음**(암묵 완료 — [[F4_중단_인수인계]], F5부터 정상 진행되고 F9a diff-0로 사후 확증됨). F7(스킬메뉴 5스킬+쿨다운, F7a)은 개발+verifier 실증 완료(2026-07-16)이나 **★필수 오너 라이브 확인(스킬 4종 사용감)은 아직 미실행**(아침 육안으로 이월, [[야간F7a_스킬메뉴_완료]] §5) — F7 게이트도 F4처럼 형식상 미종결 상태로 후속(F7b·F8·F9a)이 비블로킹 진행됨. **A1 DoD 잔여 2건**(2026-07-17 갱신): ① **S1 오라클 런**(even-trade, [[SPD원장_오라클_v1]] 20유닛턴과 전 행 diff 0 — 오라클-diff 방식 확정, 파트2 SPD 턴순서가 F9a 구원장을 무효화해 재수집 필요) ② F9b 오너 육안 풀플레이(미실행) — 상세는 §검증방침. **F7b(3층 데이터 인터프리터, 골격 완료·청크④~⑩ 잔여)는 additive-only 설계라 이 DoD를 막지 않음**([[F7b_인터프리터_진행상황_인계]] §3-④). 상태이상+AoE 계약 F4/F5/F7 선병합 완료(2026-07-14, [[상태이상_확정]]). F3(HP 게이지)는 U단계로 완결(2026-07-15, [[U단계_HP게이지_UMG_실장]]). F5-1(사망·승패 판정) 게이트 통과(2026-07-15, [[F5-1_완료]] — bBattleOver 단일관문). F5-2(죽은 유닛 처리) 게이트 통과(2026-07-15, [[F5-2_완료]] — 턴스킵·DYING·ClickBox·ResetForBattle 3청크+스킵즉시화, 오너 실플레이 검증. End버튼 재전투 배선 이월). 야간 세션(2026-07-16) — ③데미지폰트([[야간③_데미지폰트_완료]], +③b 위치수정 [[야간③b_데미지폰트_위치수정_완료]])·④End버튼 재전투([[야간④_End버튼_완료]])·⑤로그보강([[야간⑤_로그보강_완료]])·F6모션데이터구동([[야간F6_모션데이터구동_완료]])·F7a스킬메뉴 실전 5스킬([[야간F7a_스킬메뉴_완료]])·F8광폭화([[야간F8_광폭화_완료]])·F9a풀회귀([[야간F9a_풀회귀_완료]]) 전부 게이트 통과(2026-07-16). F7b는 아키텍처 SSOT([[F7_스킬아키텍처_확정]]) 기준 struct 부트스트랩 TAPython 설치([[F7b_struct부트스트랩_완료]])·데이터 3층·인터프리터 골격 완료([[F7b_인터프리터_진행상황_인계]]), ResolveHit 수술은 이월. 총결산: [[야간작업_총결산_2026-07-16]]. F9a 이후 오너 지시("attack→start")로 **파트1(Attack박스→Start버튼)·파트2(SPD 턴순서)·파트3(막기·치유 연출)** 3파트 착수(2026-07-16 22시경, 승인계획서 v3, F0~F9 코드 밖 확장 트랙) — 파트1 **게이트 PASS**(2026-07-16 23시경, [[파트1_Start_진행]] §9) — Director 직접검증(GRAPH 핀 원문)·자산·PIE·오너 육안 5건·T1 실증 전부 확인, F9a 23턴 diff-0(P1-R01)은 오너 결정으로 파트2 이월. 파트2(SPD) 착수 가능. 진행 서사·오너 결정·잠복 결함·Director 자기정정: [[파트1_Start_진행]]. 파트2 착수 — **핵심 게이트 PASS**(2026-07-17 00시경, [[파트2_SPD_완료]]) — Director 직접검증 GRAPH 5/5(서브에이전트 보고 불인정 지정 5곳 전부)+보너스 2건, ★★★최강게이트 P2-P01(런타임 spd 8기 실값) 전부 확인. 41건 TC 중 10건 실행·전부 PASS, 첫8턴 순서·23턴 신규원장 등 잔여 31건은 Start 클릭이 필요해 아침 오너 확인으로 이월. 파트3 착수 — **핵심 게이트 PASS**(2026-07-17 01시경, [[파트3_연출_완료]]) — qa-critic이 착수 전에 계획서 수정안 5건 중 3건이 명세대로면 미작동임을 확정(N1 최우선 등 신규발견 14건), Director 결정 3건(Target 조달=게이팅 전용 DT 조회 신설·카메라=OR조건 보존·치유잔상=motions.csv 데이터수정) 채택 후 구현. Director 직접검증 GRAPH 6/6(P3-G01~G06, ★★★/★★ 최우선 6건)+보너스 1건 전부 확인, 자산 3파일 동일시각 mtime 갱신·컴파일 에러 0. 38건 TC 중 6건 실행·전부 PASS, 잔여 32건(GRAPH 12·PIE 11·회귀 9)은 Start+스킬선택 클릭이 필요해 아침 오너 확인(막기·치유·회귀 4항목)으로 이월. [[언리얼_MCP_실전노하우]] §34에 2차 정정 1건(`is_dirty()` 거짓양성 — is_dirty 신뢰불가 재확인)+신규 함정 2건((84)(85)) 반영. 파트4(유닛 라벨+치유 제자리+힐 표시) 착수 — **게이트 PASS**(2026-07-17, [[파트4_라벨힐_완료]]) — 오너 육안 4항목 전부 PASS(①메뉴 라벨 `직업|이름` 형태+턴마다 갱신 ②치유 제자리 casting+`+33`초록 ③만피치유 `+0`초록 ④회귀 3종: 막기제자리/공격걸어나감/빨간데미지)+Director 직접검증 GRAPH 8건(작업1 라벨 FormatText 체인: 인자 핀 실생성·인자0/1 소스·RowNotFound 폴백)+런타임조인 P4C-15(레벨 8기 CharName↔SpdOverride 전수 정합) 전부 확인, 컴파일 0에러·디스크 저장 실증(5개 애셋). 59건 TC 중 23건 PASS·6건 F9b(오너 풀플레이) 이월·30건 코어게이트 흡수+A2 회귀스위트 이월(qa-critic 재분류). 승인 계획서 원문(humble-purring-glacier.md)은 파트4 작업 중 덮어쓰기로 소실 — git 내부 raw/ 완료 문서+커밋·태그가 파트1~4의 SSOT로 승격. [[언리얼_MCP_실전노하우]] §34에 신규 함정 1건((86), BlueprintTools 노드 refPath는 K2Node_ 접두어 필수) 반영. 청사진.md DoD에 "+F9b 오너 확인" 조항 추가(plan.md판 기준 통일). **파트4 봉인 완료(2026-07-17, 커밋 `81dcb47`·태그 `save/파트4_라벨힐`)**. SPD 턴순서 오라클(3자 독립유도 일치 — balance/Director/qa 0-diff, 총 20유닛턴)이 확정돼 [[SPD원장_오라클_v1]]로 봉인(커밋 `66659db`·태그 `save/SP2a_오라클`) — 오너 S1 검증(전 행 diff 0) 대기.
