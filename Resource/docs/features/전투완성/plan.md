...
## 파트1·2·3 — Start버튼·SPD 턴순서·막기치유 연출 (F9a 이후 확장, 2026-07-16 착수)

> F9a(풀회귀) 게이트 통과 후, 오너 지시("attack → start로 변경")를 계기로 착수한 후속 3파트. 위 F0~F9 스테이지 코드표 밖의 별도 트랙이며, **승인 계획서**(`C:\Users\user\.claude\plans\humble-purring-glacier.md`, v3, 전 에이전트 8명 리뷰 반영, git 외부)가 SSOT다. 진행 서사·오너 확정 결정·검증이 잡은 잠복 결함(Spd=0 레이스·ShowCancel 콜리전 부재·END-02 위증)·Director 자기정정은 [[파트1_Start_진행]] 참고, TC 46건은 [[파트1_Start_TC]] 참고.
