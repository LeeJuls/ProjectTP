...
feature: 기본전투무대
status: PASS
status_note: 완료 (F풀테스트 잔여)
updated: 2026-07-05
---

# 📋 기본전투무대 세부 plan

> 청사진: [[features/기본전투무대/청사진|청사진]] · 프로세스: [[개발_워크플로우]] · 로그: [[진행로그]]
...
> ⚠ **이 좌표는 초기값이다(2026-08-11 확인).** 이후 S3에서 Y=-6700(A2 둔덕 회피), S5에서 Y=-7000대 재이동 + 3cm 계단 스태거가 적용됐다. **현재 유효값은 [[진행로그]] S5 기록**이다.
...
- **쿼드 회전**(⚠ S2 발견): `SM_SpriteQuad`는 XY평면(두께0)에 누워있음 → **roll=90 필수**(정면 기립). rotation=(0,0,0)이면 카메라 정면에서 얇은 선. ~~yaw는 카메라 정면 응시 유지.~~ → ❌ **2026-08-11 정정**: 최종은 **yaw=0 · roll=90**이다. yaw=90 조합은 옆면화 버그였다. 근거: [[raw/S3_무대배치]] §7
...
> 표기: `[단계][ID] 조건 → 기대결과 | 상태`. 상세 "깨지는 시나리오"는 [[features/기본전투무대/raw/P2_TC설계|P2_TC설계]].
