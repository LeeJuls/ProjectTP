...
## 0. 증상 → 함정 색인 (문서구조_개선plan v4 6-a — 자동생성 아님, 손으로 정리·유지)

★**이 절만 추가된 것이다 — §1 이하 본문은 무수정.** 43개 절이 "학습한 시점 순"이라 *"X 증상을 고치는 법"*을
찾으려면 절 전부를 읽어야 했다. 아래는 그 문제를 푸는 증상별 진입점이다.

★**ID 표기 규칙(이 표 안에서만)**: 본문은 함정 번호를 `①`~`㊿`(1~50)과 `(51)`~`(108)` 두 표기로 섞어 쓴다
(둘 다 안정 ID — 절대 바뀌지 않는다). 이 색인은 **전부 `함정(N)` 아라비아 숫자로 통일**해 가리킨다
(예: 본문 `함정 ㉓` = 이 표의 `함정(23)`, 본문 `함정 (95)` = `함정(95)` 그대로). 본문 표기 자체는 바꾸지 않았다 —
`Ctrl+F`로 원문을 찾을 땐 위 대응표대로 순환기호(①…⑳, ㉑…㉟, ㊱…㊿)나 `(NN)`을 검색할 것.

| 증상 | 함정 |
|---|---|
| MCP 브릿지가 응답하지 않는다 / 멈춘다(hang) | 함정(23), 함정(77) |
| 노드 검색·생성이 실패한다(`find_node_types`/`create_node`) | 함정(1), 함정(9), 함정(10), 함정(14), 함정(15), 함정(16), 함정(42), 함정(86), 함정(87), 함정(88), 함정(104) |
| `set_properties`가 일부 필드만 반영된다 | 함정(3), 함정(11), 함정(55), 함정(90) |
| PIE 중 조회값이 실제와 다르다(stale) | 함정(6), 함정(51), 함정(64), 함정(70), 함정(85) |
| 저장(`save_assets`)·dirty 표시가 안 먹는다 | 함정(20), 함정(29), 함정(53), 함정(81), 함정(82), 함정(83), 함정(102) |
| PIE 틱이 멈추거나 느리다(tick rate/throttle) | 함정(7), 함정(95), 함정(96), 함정(97), 함정(98), 함정(100), 함정(105) |
| 캡처·스크린샷 결과를 믿을 수 없다 | 함정(12), 함정(13), 함정(48), 함정(103) |
| `delete_node`가 그래프를 파괴한다 | 함정(32), 함정(35) |
| 비균등 스케일+회전 자식이 뒤틀리거나 사라진다 | 함정(2), 함정(22), 함정(24), 함정(26), 함정(27), 함정(54) |
| UserDefinedStruct·DataTable 조작(TAPython/`ResolveHit`) | 함정(29), 함정(52), 함정(53), 함정(72), 함정(73), 함정(74), 함정(75), 함정(76) |
| UMG·위젯 클릭이 안 먹는다 | 함정(8), 함정(28), 함정(67), 함정(68), 함정(71) |
| Windows 데스크톱 자동화(PowerShell/Windows-MCP)가 실패한다 | 함정(47), 함정(48), 함정(61), 함정(65) |
| 한국어 에디터 로케일 때문에 오탐·오진이 난다 | 함정(49), 함정(79), 함정(104) |
| 가짜 GREEN(허위 성공) 검증 함정 | 함정(44), 함정(59), 함정(60), 함정(62), 함정(66), 함정(69), 함정(97) |
| latent 노드·exec 흐름이 꼬인다 | 함정(4), 함정(17), 함정(18), 함정(36), 함정(107) |
| bool 변수 Get/Set 노드 이름이 다르다(`b` 접두어 탈락) | 함정(16), 함정(42) |
| MCP 툴 병렬 호출이 위험하다(직렬 실행 제약) | 함정(5), 함정(23), 함정(33) |
| 참조 형식(refPath)이 도구군마다 다르다 | 함정(14), 함정(85), 함정(86) |
| 디버그/스캐폴드 리터럴 값 원복을 잊었다 | 함정(39), 함정(40) |
| 완료보고서·서술을 게이트 증거로 오인했다 | 함정(37), 함정(62) |
| 노드 생성 문자열의 카테고리·이름이 서로 다른 로캘로 섞여 검색이 헛돈다 | 함정(109) |
...
**원인**: `ResolveHit`의 §8 판정 스켈레톤 진입 첫 노드(step0)는 `GetSkillCooldown(Attacker, SkillId) > 0`이면 데미지 계산 전 조기 return하는 **쿨다운 가드**다([[features/전투완성/plan|plan]] §F4 4-2 step0, [[F4_중단_인수인계]] §3 `ResolveHit` 함수 목록). 이 가드는 F4에서 이미 구현됐지만, 그 가드가 전제하는 **쿨다운 감소(매 턴 −1 스윕)는 F5의 TS3로 예정만 돼 있을 뿐 아직 구현되지 않은 상태**였다([[F5_TC]] BLOCKER-4가 F5 착수 전 바로 이 공백을 지적한 항목). 즉 "가드는 있고 소비(감소)는 없는" 반쪽짜리 기능이 라이브 데이터 위에 얹혀 있었다. 함정㊴로 전원이 베기(쿨다운1)를 매 턴 사용하는 상태였으므로, 라운드1에 8유닛 전원이 쿨다운=1을 세팅하고 그게 영원히 안 풀려 라운드2부터 전원이 가드에 걸려 데미지·로그 없이 걸음만 반복했다.
...
## 29. 전투완성 야간④(End버튼 재전투)·야간⑤(로그보강)에서 확정된 노하우 (2026-07-16)

> 배경: 야간④(End버튼 재전투 배선, [[야간④_End버튼_완료]])와 야간⑤(로그 스키마 보강, [[야간⑤_로그보강_완료]]) 두 단계에서 발견한 BP 컴포넌트·MCP 그래프 편집·오진 방어 계열 함정 5건을 확정한 세션. §28에서 이미 유니코드 원문자가 ㊿(50)에서 소진돼 괄호숫자로 전환했으므로, **(54)부터 이어서 번호를 매긴다.**

### 함정 (54) — 레벨 기배치 액터에 BP 컴포넌트를 신설하면 구조(SCS)는 전파되나 템플릿 프로퍼티 값은 소급 전파되지 않는다(함정㉔의 TextRender 전반 확장)
**증상**: `BP_AttackButton`에 `LabelEnd`(TextRenderComponent)를 신설하고 컴포넌트 템플릿에 텍스트·스타일 값을 세팅했으나, **레벨에 이미 배치된 인스턴스**에서는 반영되지 않았다. §20 함정㉔이 `bAbsoluteLocation`/`bAbsoluteRotation`/`bAbsoluteScale` 3필드 한정으로 확인했던 "CDO 세팅이 기배치 인스턴스에 소급 전파되지 않는다"는 현상이, 이번엔 **컴포넌트 신설 자체 + 텍스트/스타일 프로퍼티 전반**으로 그대로 재현됐다.
**해법**: 새 컴포넌트를 BP에 추가한 뒤 값을 세팅할 때는 템플릿(CDO) 세팅만으로 끝내지 말 것. `find_actors`로 얻은 **각 레벨 인스턴스에도 동일 값을 개별 재적용**하는 것이 표준 절차다 — 구조(SCS 트리에 컴포넌트가 존재하는지)는 전파되지만 **값은 전파되지 않는다**는 구분을 항상 의식할 것.
**출처**: [[야간④_End버튼_완료]] §3(수정 이력②).

### 함정 (55) — `ObjectTools.set_properties`의 부분반영 버그가 스칼라 프로퍼티 조합에서도 재현된다 — §7 함정③의 "인라인 구조체 한정" 서술은 틀렸다
**증상**: `LabelEnd`(TextRenderComponent)에 `worldSize`(float)와 `bVisible`(bool)을 한 호출로 같이 세팅했더니, 재조회 시 **둘 중 하나만 반영**되고 나머지는 이전 값을 유지했다. §7 함정③은 이 부분반영 버그를 "Vector/Rotator 같은 인라인 구조체 프로퍼티에만 국한"이라고 명시했었는데, `worldSize`·`bVisible`은 둘 다 순수 스칼라라 그 서술과 정면으로 상충한다.
**해법**: **§7 함정③의 "인라인 구조체 한정"이라는 제한 서술은 폐기**한다 — 스칼라 조합을 포함해 다필드를 한 번에 `set_properties`로 세팅한 뒤에는 예외 없이 **`get_properties`로 재조회**해 전 필드 반영을 확인하고, 미반영 필드는 **단독으로 별도 호출**한다.
**출처**: 이번 세션(야간④) `LabelEnd` 세팅 중 재현.

### 함정 (56) — `ProgrammaticToolset` 스크립트가 중도에 예외를 던지면 그 앞줄의 `create_node` 효과도 커밋되지 않을 수 있다(2회 관측)
**증상**: 배치 스크립트 안에서 여러 `create_node`를 순서대로 실행하다가 뒤쪽 줄에서 예외가 나면, 그 이전 줄들이 이미 성공 반환값을 줬음에도 불구하고 **재조회 시 앞줄이 만든 노드가 그래프에 없는 사례**가 2회 관측됐다.
**해법**: 그래프를 실제로 바꾸는(노드 생성·삭제·배선) 작업은 §23 함정㉝(MCP 툴은 게임 스레드 직렬 실행)에 이어 **개별 호출로 쪼개는 편이 안전**하다. 배치 스크립트(`ProgrammaticToolset`)는 조회성 작업(§2 "8기 일괄 이동·MI 일괄 조회·지형 트레이스 루프" 등 읽기 위주)에 한정하고, 그래프 수술은 한 스텝마다 결과를 재확인하며 진행할 것.
**출처**: 야간④·야간⑤ 그래프 배선 작업 전반.

### 함정 (57) — 신설 컴포넌트를 기존 상태함수들이 모른다 — Show*/Hide* 계열 상태 토글 함수 집합에 컴포넌트를 추가하면 모든 형제 함수에 대칭 추가까지가 한 작업
**증상**: `BP_AttackButton`에 `LabelEnd`를 신설하고 `ShowEnd()`에서 `SetVisibility(true)`만 걸었더니, 기존 `ShowAttack`/`ShowCancel`/`HideAll` 3개 함수가 `LabelEnd`의 존재를 몰라 꺼주지 않았다 — End 화면에서 Attack으로 전환한 뒤에도 `LabelEnd`("End")와 `Label`("Attack")이 동시에 켜진 채 겹쳐 보였다.
**해법**: 상태 토글 함수 집합(Show*/Hide* 계열)에 새 컴포넌트를 추가할 때는 **그 컴포넌트를 노출하는 함수 하나만 만들고 끝내지 말고, 나머지 형제 함수 전부에 대칭적으로 끄기(`SetVisibility(false)`)를 추가**하는 것까지가 한 작업이다. 같은 세션에서 발견된 콜리전 재활성 비대칭(`HideAll`이 끄기만 하고 `ShowAttack`만 다시 켜는 쌍으로 관리되던 것 — [[야간④_End버튼_완료]] §3 수정이력①의 `ClickBox` 잠김 사고)도 동일 계열의 함정이다. **상태를 세팅하는 함수와 해제하는 함수는 항상 짝으로 감사할 것.**
**출처**: [[야간④_End버튼_완료]] §3(수정 이력①·③).

### 함정 (58) — 오진 방어: `collisionResponses.responseArray`는 프로파일 기본값과의 diff만 나열한다 — 채널 부재 ≠ 무응답
**증상**: 중간 실증자가 특정 유닛 클릭 무반응을 "`ClickBox`의 Visibility 콜리전 채널 누락 리그레션"으로 오진해 보고했다. 근거는 `get_properties`로 조회한 `collisionResponses.responseArray`에 `Visibility` 채널이 안 보인다는 것이었다.
**원인**: `responseArray`는 **콜리전 프로파일 기본값과 다른 항목만** 나열하는 diff 표현이다 — 어떤 채널이 배열에 없다고 해서 그 채널이 무응답(Ignore)이라는 뜻이 아니라, **프로파일 기본값(대개 Block)을 그대로 따른다**는 뜻일 수 있다. Director가 직접 재조회해 실제로는 `Visibility:Overlap`이 프로퍼티에 존재함을 확인, 오진을 반증했다. 실제 원인은 클릭 좌표 오조준(로그에 `ignored (same team or self)` 도달 — 클릭 자체는 처리됐으나 대상 판정이 어긋남)과 창 가림이었다.
**해법**:
1. `responseArray` 조회 시 **부재를 무응답으로 해석하지 말 것** — 항상 프로파일 기본값과 함께 판단한다.
2. **PIE 실측 이력이 있는 좌표는 재계산으로 기각하지 말 것** — 검증된 좌표가 이론적 재계산보다 우선한다.
3. 클릭 무반응 진단 시 **`UnitClicked: ignored` 류의 "도달했지만 거부된" 로그부터 확인**할 것 — 도달 여부와 미도달 여부를 가르는 것이 진단의 절반이다.
4. 서브에이전트가 CONFIRMED로 보고해도 **교차 증거가 모순되면 Director가 직접 진단**한다.
**출처**: [[야간④_End버튼_완료]] §5(오진 사례 기록).

---

## 30. 전투완성 야간F6(모션 데이터구동)에서 확정된 노하우 (2026-07-16)

> 배경: 야간F6(`PlayAttack` 리터럴 → `DT_Skills`·`DT_Motions` 데이터구동 전환, [[야간F6_모션데이터구동_완료]]) 게이트에서 검출한 가짜 GREEN 2건과, 같은 세션의 데스크톱 자동화·완료보고 신뢰 계열 함정 2건을 합쳐 총 4건을 확정했다. §29에 이어 **(59)부터 번호를 매긴다.**

### 함정 (59) — 세터-소비자 시간차 함정: EventGraph 병렬 경로에서 변수 Set이 소비자보다 늦으면 무음 폴백과 결합해 가짜 GREEN이 된다
**증상**: EventGraph에 시간차를 둔 병렬 경로(이번 사례: 걸음/모션 재생 ~0.4s vs 판정 처리 ~1.1s)가 공존할 때, 공유 변수(`PendingSkillId`)의 `Set`을 늦은 경로(판정, ~1.1s)에 두었더니 이른 경로(모션 조회, ~0.4s)의 소비자가 항상 그 변수의 **기본값**(0)을 읽었다.
**원인**: BP EventGraph는 여러 실행 체인이 시간축 상 겹쳐 흐르는데, "이 변수는 이 함수 안 어딘가에서 세팅되니 안전하다"는 직관이 **실행 순서(시각)를 보장하지 않는다** — 특히 하류 소비가 무음 폴백(`RowNotFound`→폴백값)과 결합하면 오배선이 겉보기엔 "정상 동작"으로 위장돼 발견이 더 어려워진다(§30 (60) 참고).
**해법**: **공유 변수의 Set은 이벤트 진입 직후가 원칙**이다 — 그 변수를 읽는 모든 소비자보다 시간상 반드시 앞서도록 배치한다. 지연 경로(Delay·모션 재생 등) 뒤에 Set을 두는 배선을 보면 "이 변수를 그 경로 밖에서도 읽는 소비자가 있는가"부터 점검할 것.
**출처**: [[야간F6_모션데이터구동_완료]] §2(가짜 GREEN ②).

### 함정 (60) — 연쇄 가짜 GREEN: 상류 조회 실패의 폴백값이 하류 조회의 유효 키와 겹치면 "정상 성공"으로 위장한다 — 검증은 반드시 비기본값 관통 테스트로
**증상**: 상류 조회(`DT_Skills`, `SkillId=0`)가 `RowNotFound`로 실패해 폴백값(`motionRow=5`)을 반환했는데, 그 폴백값이 하류 조회(`DT_Motions`)에서는 **실존하는 유효한 행**이라 `FrameCount=6.0`이 정상적으로 반환됐다 — 상류가 실패했다는 사실 자체가 하류 결과에서 완전히 지워진다.
**위험**: 이 상황은 상류·하류를 각각 따로 확인하면 둘 다 "에러 없음"으로 보이고, 최종 출력값(6.0)마저 기대값과 우연히 일치(Atk1의 실제 FrameCount가 6)하면 **항등 케이스만으로는 영원히 미검출**된다.
**해법**: 이런 다단 조회 체인의 검증은 반드시 **비기본값 관통 테스트**로 한다 — 폴백값과 다른 값(이번 사례: `motionRow=10`)을 의도적으로 흘려보내, 그 값이 체인 끝까지 손실 없이 도달하는지를 직접 확인한다. 항등 케이스(폴백=정답인 경우)는 판별력이 0인 테스트다.
**출처**: [[야간F6_모션데이터구동_완료]] §2(가짜 GREEN ②) — (59)와 같은 사고의 두 측면이다(원인=(59), 은폐 메커니즘=(60)).

### 함정 (61) — 동일 프로세스 복수 최상위 창 경쟁: BP 에디터 창이 메인 창과 겹치면 Get-Process 첫 창 활성화가 오식별한다
**증상**: 언리얼 에디터는 레벨 에디터 메인 창과 BP 에디터 창이 **같은 프로세스의 별개 최상위 창**으로 동시에 존재할 수 있는데, `Get-Process`가 반환하는 첫 창을 그대로 활성화하면 의도한 창(레벨 에디터)이 아니라 BP 에디터 창 등 다른 최상위 창이 잡히는 오식별이 발생한다. `SetForegroundWindow` 단독 호출로는 이 문제를 해결하지 못한다.
**해법**: **`EnumWindows`로 타이틀을 직접 특정**한다(이번 사례: `"projectTP*"` 패턴의 레벨 에디터 창). 그 외 잡히는 스트레이 창은 `SW_MINIMIZE`로 치우고, 전면화는 **ALT 키 트릭 + `BringWindowToTop`** 조합으로 안정화한다.
**재사용 자산**: scratchpad `list_windows.ps1` · `activate_ue2.ps1`.
**절차화**: 클릭 자동화(§6 데스크톱 자동화 계열) 세션을 시작할 때는 이 창 정리를 **절차 0번**으로 둔다.

### 함정 (62) — 개발 완료보고의 배선 서술 불신 원칙: "False→직결" 같은 텍스트 서술은 게이트 증거가 아니다
**증상**: F6 작업 중간 보고에서 `frameCount≥1` 가드의 특정 분기를 "`False`→직결"(바로 종단)로 배선했다고 서술했으나, 실제 그래프는 "`False`→`6.0` 재대입"(가드 반전 — [[야간F6_모션데이터구동_완료]] §2의 가짜 GREEN ①)이었다 — 서술과 실그래프가 상이했다.
**해법**: 핵심 분기·배선의 완료 판정은 서술을 신뢰하지 말고 반드시 `get_node_infos`의 **`connected_pins` 원문**으로 확인한다. 서술만으로 이뤄진 완료 보고는 게이트 통과의 증거가 될 수 없다 — 이 원칙은 §29 (58)(서브에이전트 CONFIRMED 보고도 교차증거 모순 시 Director 직접 진단)의 연장선이며, 대상이 "값"에서 "배선 그 자체"로 확장된 것이다.
**출처**: [[야간F6_모션데이터구동_완료]] §2(가짜 GREEN ①).

---

## 31. 전투완성 야간③b(데미지폰트 위치수정)에서 확정된 노하우 (2026-07-16)

> 배경: 야간③b(카메라 전환 시 데미지 폰트 위치 이탈 수정, [[야간③b_데미지폰트_위치수정_완료]]) 게이트에서 확정한 BP 외부함수 배선·PIE 프로퍼티 조회·데스크톱 자동화 계열 함정 3건. §30에 이어 **(63)부터 번호를 매긴다.**

### 함정 (63) — 외부 BP의 raw 멤버변수 Set 노드는 `create_node`로 생성 불가 — Set 래퍼 함수 신설 후 함수호출 노드로 우회
**증상**: 다른 블루프린트(`WBP_DamageNumber`)에 정의된 raw 멤버변수(`TargetWorldLoc`)를 대상으로 Set 노드를 `create_node`로 생성하려 하면 거부된다 — `find_node_types`는 해당 변수의 Set 노드 타입을 정상적으로 찾아내지만, 그 문자열로 `create_node`를 호출하면 실패한다. **탐색(find)과 생성(create) 사이의 툴 간극**이 실측됐다.
**해법**: 원본 변수에 Set 노드를 직접 꽂으려 하지 말고, 대상 BP에 그 변수를 세팅하는 **Set 래퍼 함수를 신설**(`SetTargetWorldLoc`)한 뒤, 호출 측에서는 §11 함정⑨가 확립한 "검증된 외부 함수호출 패턴"(다른 블루프린트의 함수를 함수호출 노드로 호출)을 그대로 적용한다.
**출처**: [[야간③b_데미지폰트_위치수정_완료]] §3(위젯 자가 재투영 구현 — Manager→`WBP_DamageNumber.SetTargetWorldLoc` 배선).

### 함정 (64) — PIE 중 게임플레이 bool 프로퍼티 조회는 stale할 수 있다 — 판정은 로그·화면 라벨 등 교차 소스로(함정(51)의 bool 확장)
**증상**: PIE 실행 중 `bCamActionEnabled` 같은 게임플레이 bool 프로퍼티를 조회하면, 실제 토글 상태 변화와 무관하게 **`true`로 고정 반환**되는 사례가 나왔다.
**원인**: 함정(51)(§28)이 트랜스폼 조회 한정으로 확인했던 "PIE 중 프로퍼티 조회값 불신"이 **bool 프로퍼티로도 그대로 확장**되는 것으로 관찰된다.
**해법**: 토글류 상태 판정에 프로퍼티 조회값을 단독 신뢰하지 말고, **로그(`CamToggle:` 등 명시적 이벤트 로그) · 화면 라벨**처럼 조회 API를 거치지 않는 교차 소스로 판정한다.
**출처**: [[야간③b_데미지폰트_위치수정_완료]] §2(카메라 ON/OFF 진단 매트릭스).

### 함정 (65) — 외부 프로세스 창이 활성화-클릭 사이 시간차에 포그라운드를 가로챈다 — 활성화+클릭+캡처를 단일 PowerShell 호출로 원자화(함정(61)의 프로세스 간 확장)
**증상**: 창을 활성화한 시점과 실제 클릭 시점 사이에 시간차가 있으면, 그 사이 **다른 애플리케이션 창이 포그라운드를 탈취**하는 사고가 **실제로 2회 발생**했다(무효 캡처 1건·클릭 유실 1건).
**원인**: 함정(61)(§29)이 확인한 "동일 프로세스 내 복수 최상위 창 경쟁"이 이번엔 **프로세스 간(외부 애플리케이션)** 으로 확장된 사례다 — 활성화와 클릭 사이에 존재하는 모든 시간차가 탈취 창구가 된다.
**해법**: **활성화+클릭+캡처를 단일 PowerShell 호출로 원자화**하는 것이 표준이다 — 그 사이에 MCP 왕복 등 지연 구간을 두지 않는다. §28 함정㊽(로컬 .NET 버스트 캡처)·§29 함정(61)(EnumWindows 창 특정)과 결합해 "창 특정 → 활성화 → 클릭 → 캡처"를 한 스크립트 안에서 끊김 없이 실행하는 것이 이번 사례로 재확인됐다.
**출처**: [[야간③b_데미지폰트_위치수정_완료]] §4(픽셀 색상 매칭 검증 캡처).

---

## 32. 전투완성 야간F7a(스킬메뉴 실전배선)에서 확정된 노하우 (2026-07-16)

> 배경: 야간F7a(스킬메뉴 실전 배선 — 하드코딩 제거·기존 5스킬 실전·쿨다운, [[야간F7a_스킬메뉴_완료]]) 게이트에서 E2E 6차 반복 끝에 검출·수정한 결함 4건(+보너스 1건)과, 프로젝트 최초로 UMG를 실전 도입하며 확정된 함정을 합쳐 총 6건을 확정했다. §31에 이어 **(66)부터 번호를 매긴다.**

### 함정 (66) — `FunctionEntry.then` 미연결은 시스템 함정으로 확정(한 야간 세션 내 3재발) — 컴파일 0에러가 "본체 실행됨"을 보장하지 않는다
**증상**: `create_node`로 함수 본체 그래프를 만들 때 `FunctionEntry` 노드의 `then` 실행핀을 본체 첫 노드에 연결하는 과정을 빼먹기 쉽다. 컴파일은 **0에러**(경고만)로 통과하므로, 함수가 "호출은 되지만 본체가 전혀 실행되지 않는" 상태가 겉으로 전혀 드러나지 않는다 — 이번 세션에서만 **3회 재발**했다: `Menu.SetActiveUnit`(lazy-init 체인, 결함①) · `Manager.NotifySkillSelected`(결함④) · 카메라 컷 퇴화 분기의 `then`(결함⑤, 조건분기 변종 — 발생기전이 달라 별도로 §32 함정(69)에 카탈로그).
**해법**: 함수·분기 신설 완료보고에는 **해당 실행핀의 `connected_pins` 원문**을 의무 첨부한다. "No execute pin found" 경고는 빈 스텁에도 동일하게 뜨므로 미연결의 신호로 쓸 수 없다 — 진위는 반드시 핀 조회(`get_node_infos`)로만 판별한다.
**출처**: [[야간F7a_스킬메뉴_완료]] §2(결함①④, 관련 ⑤).

### 함정 (67) — UMG를 프로젝트에 처음 실전 도입하면 `SetInputMode(GameAndUI)`가 필수다 — 월드클릭 전용 프로젝트는 GameOnly라 Slate가 클릭을 원천 미수신
**증상**: 지금까지 월드 액터(Attack박스 등) 클릭만으로 조작하던 프로젝트에 처음으로 진짜 UMG 버튼(스킬메뉴 Row)을 배선했더니, 버튼이 클릭에 전혀 반응하지 않았다 — 호버 이벤트조차 발생하지 않았다.
**원인**: 프로젝트 전체에 `SetInputMode` 호출이 **0건**이었다 — 기본값 GameOnly 모드에서는 Slate(UMG) 레이어가 마우스 입력을 아예 받지 못한다. `bShowMouseCursor`·`bEnableClickEvents`를 켜는 것만으로는 부족하다.
**해법**: `InitBattle`에 `SetInputMode(GameAndUI)` 호출을 추가한다. 진짜 UMG 버튼을 프로젝트에 처음 도입하는 시점엔 이 세팅부터 확인할 것.
**출처**: [[야간F7a_스킬메뉴_완료]] §2(결함③).

### 함정 (68) — 목업 위젯의 히트테스트 잔재가 실전 전환 후에도 클릭을 통과시킨다
**증상**: 표시 전용(목업)으로 만들었던 스킬메뉴 Row 위젯을 실전 배선으로 전환했는데, Row 인스턴스 등 **8개 위젯**에 `SelfHitTestInvisible`류 설정이 그대로 남아 클릭이 통과(투과)해버렸다.
**해법**: 정석 히트테스트 구성으로 되돌린다 — **컨테이너는 `SelfHitTestInvisible`**, **버튼은 `Visible`**, **버튼 내부 텍스트 등 요소는 `HitTestInvisible`**. 목업을 실전으로 전환할 때는 이 3단 구성을 항상 재확인할 것.
**출처**: [[야간F7a_스킬메뉴_완료]] §2(결함②).

### 함정 (69) — 조건 퇴화 분기의 잠복 단선은 새 기능이 그 조건을 처음 밟을 때까지 수개월 무증상이다(함정(66)의 조건분기 변종)
**증상**: 카메라 컷의 "거리<1.0" 퇴화 분기가 미연결 상태였는데, **F4 단계부터** 8기 전원의 실전 배치 거리가 항상 1.0을 훨씬 초과해 이 분기가 **단 한 번도 실행되지 않았다** — 그래서 미연결이 여러 단계를 거치도록 드러나지 않았다. SELF 대상 스킬(캐스터-대상 거리=0)이 처음으로 이 분기에 도달하면서 소프트락으로 발현됐다.
**원인**: 실전 데이터로는 도달 불가능한 조건 분기는 미연결이어도 컴파일·정상 플레이 양쪽에서 아무 신호를 내지 않는다 — 함정(66)의 "본체 미실행 무음"이 조건 분기 레벨에서도 성립하는 사례.
**해법**: 카메라 미발동 경로를 합류시키는 연결 1개로 수정. **분기를 신설할 때는 양쪽 출구(정상 경로·퇴화 경로) 모두의 연결을 핀 원문으로 확인**한다 — "지금까지 안 터졌다"는 "연결돼 있다"의 증거가 아니다.
**출처**: [[야간F7a_스킬메뉴_완료]] §2(결함⑤ 보너스 — SELF 스킬 소프트락).

### 함정 (70) — PIE 중 UMG 위젯 프로퍼티 조회도 stale하다 — 함정(64)의 UMG 확장
**증상**: 스킬메뉴 Row 위젯의 `SkillId`/`bOnCooldown` 프로퍼티를 `get_properties`로 조회하면 실제 상태 변화와 무관하게 **항상 기본값**만 반환됐다.
**원인**: 함정(64)(§30)가 게임플레이 bool 프로퍼티 한정으로 확인했던 "PIE 중 프로퍼티 조회값 불신"이 **UMG 위젯 프로퍼티**로도 그대로 확장되는 사례다.
**해법**: 위젯 프로퍼티 조회값을 단독 신뢰하지 말 것. 권위 소스는 **액터 프로퍼티**(`SkillCooldowns` 등 Manager 측 원본 데이터)와 **화면 증거**(회색조·배지 등 렌더 결과)로 판정한다.
**출처**: [[야간F7a_스킬메뉴_완료]] §3(판정표 — 쿨다운 검증).

### 함정 (71) — 쿨다운 중인 행의 클릭은 Slate 레이어에서 이미 소진된다 — 함수 내 거절 로그는 도달 불가 안전망
**증상**: 쿨다운 중인 스킬 Row를 클릭해도 `NotifySkillSelected` 함수 내부의 거절 로그(쿨다운 가드)가 한 번도 찍히지 않았다.
...
> 배경: F9a 풀회귀 이후 착수한 파트1(Attack 박스→Start 버튼 전환, [[파트1_Start_TC]]) 착수 전 실측 단계·그 직전 에디터 재부팅 대응·이후 구현 단계에서 확정한 `call_tool` 호출형식·기존 오진 정정·StringTable 강제dirty 해소/한계·uasset 검증 계열 함정 6건((78)~(83)). 근거: 계획서 `C:\Users\user\.claude\plans\humble-purring-glacier.md` §착수 전 필수 실측 + [[파트1_Start_진행]](구현 단계 실측, §9-8). §33에 이어 **(78)부터 번호를 매긴다.**
> ▶ **(84)~(85) 추가(2026-07-17)**: 같은 승인계획서 트랙의 이후 세션(파트2 SPD·파트3 연출)에서 확정된 함정 2건을 절을 새로 열지 않고 **같은 §34에 번호만 이어서** 등재한다 — 출처: [[파트2_SPD_완료]] §6·§5 · [[파트3_연출_완료]].
> ▶ **(86) 추가(2026-07-17)**: 같은 트랙의 파트4(유닛 라벨·치유 제자리·힐 표시) 세션에서 확정된 `BlueprintTools` 노드 refPath 규약 함정 1건을 동일 방식으로 추가한다 — 출처: [[파트4_라벨힐_완료]].
...
> ★**2026-08-11 추가 — 같은 유형의 오판이 재발했다.** 함정 **(88)**(§35)에서 `InputKey`로 검색해 0건이 나온 것을 *"이벤트 노드가 없다 → 폴링으로 대체"*로 판정했는데, **`Keyboard` 카테고리로 검색하니 전부 나왔다.**
> (79)는 **로캘**(영문↔한국어), (88)은 **검색어 층위**(기능명↔팔레트 카테고리명)로 원인은 다르지만, **"검색 0건을 존재하지 않음으로 결론냈다"**는 오류의 형태가 같다.
> → **상위 원칙: `find_node_types` 0건은 "없다"가 아니라 "이 검색어로는 못 찾았다"이다.** 결론 전에 최소 **로캘(한/영)**과 **층위(기능명/카테고리명)** 두 축을 바꿔 재검색하라.
...
### 함정 (82) — uasset 바이너리 grep도 `is_dirty()` 직접조회도 "최신 저장 증명"으로는 둘 다 신뢰할 수 없다 — 한계 명시 (2차 정정)

**🚨 이 항목은 하루에 두 번 고쳤다.** 정정 이력을 숨기지 않고 그대로 남긴다 — 셋 다 사실이었고, 셋 다 "그때까지의 최선"이었다.
1. **원판**: uasset을 ASCII로 바이너리 grep하는 방법을 **"값싸게 검증하는 도구"**로 적었다(Director).
2. **1차 정정**(커밋 `2767081`): **Codex 전체 읽기전용 리뷰(FULL-003, P1)**가 *"uasset 문자열 존재와 파일 크기 유사성을 '저장 안 함' 근거로 쓰는 문서 지침이 실제 미저장 작업을 버릴 수 있음"*을 지적 → 수용. 바이너리 grep을 "**존재 증명일 뿐 최신 저장 증명 아님**"으로 한계 명시하고, "더 확실한 방법"으로 **`is_dirty` 직접 조회를 바이너리 grep보다 항상 우선**시키도록 격상했다.
3. **2차 정정(지금, 이 절)**: **그 `is_dirty` 직접 조회조차 신뢰할 수 없다는 실측이 나왔다.** 1차 정정이 "더 확실하다"며 제시한 대안이 반나절도 못 가 무너진 사례다 — 아래 참고.

**배경**(원판부터 불변): BP는 `.gitignore`로 git 추적 밖에 있어, "이 작업이 실제로 디스크에 저장됐는가"를 git diff로 확인할 방법이 없다.
```powershell
$t = [System.Text.Encoding]::ASCII.GetString([System.IO.File]::ReadAllBytes($uasset))
$t.Contains("ApplySkillEffects")   # → 디스크본에 그 "이름"의 함수가 있는가 — 그 이상은 증명 안 됨
```
**바이너리 grep의 한계(1차 정정 확정, 변경 없음)**:
- **쓸 수 있는 경우 — 부재 증명(신뢰 가능)**: grep 결과가 **0건**이면 "이 함수는 디스크에 아예 없다"는 확실한 결론이다. 문자열이 없다는 사실 자체가 증거이므로 이 방향은 안전하다.
- **쓸 수 없는 경우 — 최신성 증명(불가능)**: 문자열이 **있다**는 결과는 "그 함수가 (그 시점 디스크본에) 존재한다"만 증명하지, **"그것이 최신 작업 내용대로 저장됐다"는 전혀 증명하지 못한다.** 함수 이름은 그대로 두고 내부 배선·로직만 바뀐 경우 grep으로는 신·구본을 **구분 불가능**하다. 파일 크기 유사성도 마찬가지로 정황일 뿐이다. 이 용도로는 절대 단독 근거로 쓰지 말 것.

**🚨 2차 정정 — `is_dirty()`를 신뢰 목록에서 강등**: 파트3(연출) 구현 세션 실측(gameplay-engineer) — `AssetTools.is_dirty()`가 **`save_assets` 성공 직후에도 계속 `true`를 반환**했다. 대상 3개 에셋(`BP_BattleManager`·`BP_BattleSpawnPoint`·`DT_Motions`) **전부**에서 재현. 그런데 **동일 에셋을 재저장하니 파일 mtime·크기·SHA256이 전부 동일**(완전 no-op)했다 — 즉 **실제로는 이미 clean하게 저장된 상태였는데 `is_dirty()`가 거짓 양성(false positive)을 반환한 것**이다. 1차 정정이 "바이너리 grep보다 항상 우선"이라며 격상시킨 바로 그 대안이 무너졌다.

**✅ 지금 신뢰할 수 있는 판정 방법(승격, 이 순서로 시도)**:
1. **디스크 mtime 갱신 확인** — 가장 단순·확실. 저장 전 mtime을 기록해 두고 저장 후 갱신 여부를 직접 본다.
2. **재저장 no-op 확인** — 판단이 안 서면 다시 저장해 본다. mtime·크기·해시가 **전부 동일**하면 이미 clean(오늘 이 방법으로 위 `is_dirty()` 거짓 양성을 실제로 걸러냈다).
3. 그래도 판단이 갈리면 **자동저장본부터 먼저 백업**하고 "저장 안 함"을 선택한다(원판이 세운 안전장치, 여전히 유효 — 절차는 아래 "정직한 재평가" 참고).

**🔑 일반 원칙으로 승격**: **이 MCP 브리지의 상태 조회 API는 반환값이 실제 디스크 상태와 어긋날 수 있다.** `save_assets`가 돌려주는 `true`(§34(81) StringTable 사례 — 저장을 무음으로 스킵했는데 성공으로 응답)와 `is_dirty()`가 돌려주는 `true`(이번 사례 — 저장에 성공했는데 dirty로 응답)는 **오차 방향은 정반대지만 같은 부류의 결함**이다. **판정은 API 반환값이 아니라 항상 파일시스템(mtime·크기·해시)으로 한다.**

**정직한 재평가**(원판 사례, 결론 갱신): 2026-07-16 F7b 저장 판단은 **결과적으로는 맞았다**(이후 세션까지 문제없이 이어짐으로 사후 확인됨). 하지만 그 판단은 grep 단독이 아니라 **정황 증거 3개의 조합**이었다 — ①F7b가 만든 함수 4개 전부가 디스크본에 존재 ②여러 자동저장본 크기가 서로 동일(=그 사이 실제 작업이 없었음을 시사) ③커밋 시각이 저장 시각보다 뒤(실측: 디스크본 2,389,751바이트 vs Autosave 2,389,823바이트, **72바이트 차**). 이 중 ③(커밋 타임라인)이 실제로는 가장 강한 근거였는데, 원판 제목은 이를 "바이너리 grep이 검증 도구"라는 단일 방법론으로 뭉뚱그려 잘못 대표했다. **다음에 같은 판단을 할 때의 순서**: ①디스크 mtime 갱신 확인 → ②(불명확 시) 재저장 no-op 확인 → ③그래도 판단이 갈리면 자동저장본 백업 확보 → ④정황 증거 복수 조합. (1차 정정판의 "①dirty 직접 조회"는 2차 정정으로 폐기됐다 — `D:\unreal\_savepoints\파트1_Start_착수전\autosave_참고\`의 자동저장본 백업 사례 자체는 여전히 유효한 안전장치 선례다.)

**출처**: 2026-07-16 F7b 저장 판단(파트1 착수 직전 세션) · Codex 전체 읽기전용 리뷰 FULL-003([[Codex_전체_읽기전용_리뷰_2026-07-16]], P1) · 1차 정정: 2026-07-17(Director, 커밋 `2767081`) · 2차 정정: 2026-07-17(Director) — 파트3(연출) 구현 세션 `is_dirty()` 거짓 양성 실측, [[파트3_연출_완료]].
...
### 함정 (84) — `ProgrammaticToolset` 스크립트 전체가 단일 UE 트랜잭션이다 — 꼬리의 사소한 실패가 앞선 성공을 통째로 롤백한다
**증상**: 스크립트 **맨 끝의 사소한 호출 하나**(예: `get_node_infos` 인자 형식 오류)가 예외를 던지면, 그 앞에서 **성공적으로 실행된 `create_node`/`connect_pins`/`set_properties`가 전부 자동 Undo(롤백)** 된다. 로그 실측: `LogEditorTransaction: Undo Execute tool script`.
**2회 재현**:
1. 파트2 작업3(레벨 8기 `SpdOverride` 세팅) — `save_actor` 실패(OFPA 미해당 에러, §34(80) 인접 사례)로 앞서 성공한 `set_properties`(`C_2`=97 등)가 함께 무효화됨([[파트2_SPD_완료]] §6).
2. 파트3 SpawnPoint 1차 스크립트 — 스크립트 끝단의 사소한 오류로 앞서 만든 노드·배선이 통째로 롤백됨([[파트3_연출_완료]]).
**원인**: `ProgrammaticToolset`(`execute_tool`)은 스크립트 1회 호출을 **UE 에디터 트랜잭션 1개**로 감싼다. 트랜잭션 안에서 예외가 나면 엔진은 "부분 성공"을 허용하지 않고 **트랜잭션 시작 시점 상태로 통째 되돌린다** — 스크립트 앞부분이 몇 개의 쓰기를 성공시켰든 무관하다.
**해법**: 긴 스크립트일수록 위험 노출이 크다.
- **검증 호출을 쓰기 스크립트 안에 섞지 말 것** — `get_node_infos` 등 조회성 호출도 그 자체로 스크립트의 실패 가능성을 늘린다. **쓰기 스크립트는 쓰기만 하고, 검증은 별도 호출로 분리한다.**
- 롤백은 **조용히** 일어난다(예외 로그는 남지만 "그래서 앞선 성공까지 전부 사라졌다"는 명시적 경고가 없다) — "분명 만들었는데 없다"는 증상을 보면 이 함정부터 의심할 것. 스크립트 직후 별도 호출로 `is_dirty`/`get_node_infos` 재확인하는 습관이 유일한 조기 발견 수단이다. 단 §34(82)(2차 정정)대로 `is_dirty` 자체도 단독 신뢰는 금지 — 최종 판정은 항상 파일시스템으로.
**출처**: 2026-07-16 파트2 작업3([[파트2_SPD_완료]] §6) · 2026-07-17 파트3 SpawnPoint 1차 스크립트([[파트3_연출_완료]]).

### 함정 (85) — PIE 런타임 프로퍼티 조회의 유일한 정답 경로: 액터 refPath 형식
**증상**: verifier가 이 형식을 몰라 파트1 PIE 검증(런타임 상태 조회)을 통째로 못 하고 "MCP 액터 경로 형식 불일치"로 보고했다.
**틀린 형식들**:
- ❌ `/Temp/UEDPIE_0_map_battle_octopath...`
- ❌ `/Game/Blueprints/BP_BattleManager:UEDPIE_0_...`
**옳은 형식**:
- ✅ **`/Game/Stages/UEDPIE_0_map_battle_octopath.map_battle_octopath:PersistentLevel.BP_BattleSpawnPoint_C_2`**
  = **원래 레벨 폴더 경로 + 레벨명 앞에 `UEDPIE_0_` 접두어** + `:PersistentLevel.<액터명>`
**호출**: `ObjectTools.get_properties(instance={"refPath": "..."}, properties=["Spd","SpdOverride"])`
  ⚠ 파라미터명이 `object`/`property_names`가 **아니라** `instance`/`properties`(§2 표가 이미 확정한 규약과 동일 계열 — PIE 액터 refPath 형식만 신규).
**가치**: 이 경로로 파트2의 최강 게이트(런타임 `spd` 8기 실값, [[파트2_SPD_완료]] §3)를 직접 읽었다. qa-critic이 금지한 "에디터 스냅샷 오독"을 피하는 **유일한 수단**이기도 하다 — 에디터의 CDO/기본값이 아니라 PIE 월드의 **그 순간 실제 런타임 값**을 읽으므로.
**출처**: 2026-07-16 파트1 PIE 검증 시도(verifier) 실패 사례 · 2026-07-17 파트2 실측 성공([[파트2_SPD_완료]] §5, "§34 후보"로 이미 지목됨) · 파트3에서도 재사용([[파트3_연출_완료]]).

### 함정 (86) — `BlueprintTools` 노드 refPath는 `K2Node_` 접두어가 필수다 — `find_nodes`의 title은 한국어 표시명
**증상**: 그래프 노드를 대상으로 `get_node_infos` 등을 호출할 때 `refPath`를 접두어 없이(`<BP>:<그래프>.FormatText_0` 형태로) 넘기면 `"not valid EdGraphNode"` 에러로 실패한다.
**원인**: `BlueprintTools`가 기대하는 노드 refPath 형식은 항상 **`K2Node_` 접두어를 포함**한다 — 예: `<BP>:<그래프>.K2Node_FormatText_0`. 클래스 내부 이름(`K2Node_FormatText`)과 그래프 상의 표시 이름(한국어 "포맷 텍스트")이 서로 다르다는 점이 함정의 핵심이다.
**해법(실무 순서)**: 노드를 다룰 때는 항상 이 순서로 진행한다.
1. `get_graph(blueprint, graph_name)`로 그래프를 확인.
2. `find_nodes(graph, title="...")`로 노드를 찾는다 — 이때 `title`은 **한국어 표시명**(예: `"포맷 텍스트"`)을 써야 매치된다.
3. `find_nodes`가 반환한 `refPath`(이미 `K2Node_` 접두어가 붙어 있음)를 그대로 `get_node_infos(nodes=[{refPath}])`에 넘긴다.
직접 조합한 refPath(접두어 누락·영문 클래스명 추정)는 매번 실패한다 — 반드시 `find_nodes`가 반환한 문자열을 그대로 재사용할 것.
**출처**: 2026-07-17 파트4(유닛 라벨·치유 제자리·힐 표시) 세션, `WBP_SkillMenu:SetActiveUnit`의 `K2Node_FormatText` 노드 조회([[파트4_라벨힐_완료]]).

---

## 35. 스킬연출구조 E-S0(노드 프로브)에서 확정된 노하우 (2026-08-11)

> 배경: 스킬연출구조 E단계 스파이크 착수 전 노드 가용성 프로브(S0, [[E-S0_노드프로브_결과]])에서 확정된 함정 2건. 필수 15종 중 8종은 신규 BP에서도 그대로 성공했으나, **외부 클래스를 새로 참조하는 3종**(`B1` raw 변수 Get·`B4` `AlignSpriteToCamYaw`·`B5` `PlayHurtReaction`)이 전부 실패했고, 원인을 추적하니 §11 함정⑨의 성공 사례가 신규 BP에는 일반화되지 않는다는 것이 밝혀졌다. 이 실패가 Director의 "유닛을 참조하지 않는 대역 액터(`BP_FxLabDummy`)로 전환" 판정으로 이어졌다(경위: [[E-S0_노드프로브_결과]]). §34에 이어 **(87)부터 번호를 매긴다.**

### 함정 (87) — 검색 성공 ≠ 생성 가능: 신규 BP에서 외부 클래스를 새로 참조하는 노드는 `find_node_types`가 찾아도 `create_node`가 거부한다
**증상**: `find_node_types`(`context_pins` 포함)가 반환한 노드 타입 문자열을 그대로 `create_node`에 넣으면 **"does not exist" 에러로 거부**될 수 있다. 실측: `Variables|디폴트|GetSpriteMID` 검색은 성공했으나 `create_node`는 실패. `AlignSpriteToCamYaw`·`PlayHurtReaction` 외부 호출도 동일하게 실패 — §11 함정⑨의 해법(기존 노드 역산)과 `declaring_class` 병기를 포함한 3가지 방식 전부 실패했다.
**원인**: 경계는 **"신규(관계 없는) 블루프린트에서 외부 클래스의 변수·함수·이벤트를 새로 참조"**다. `find_node_types` 검색은 팔레트 기준으로 동작하지만 `create_node` 생성은 그래프 컨텍스트(기존 참조 관계) 기준으로 동작해 갈린다. ★§11이 성공 실증으로 기록한 `PlayAttack` 사례는 **이미 `BP_BattleSpawnPoint` 참조가 확립된 `BP_BattleManager` 내부**에서 이뤄진 것이었다 — 완전히 새 BP에서 시도하면 재현되지 않는다. 즉 §11의 해법은 신규 BP에는 일반화되지 않는다.
**해법**: 외부 참조가 필요하면 **(a)** 이미 참조 관계가 확립된 BP 안에서 작업하거나 **(b)** 참조 대상을 자기 소유 액터(대역)로 대체한다. `retarget_node_class`로 self-scope 커스텀이벤트의 클래스 참조를 외부 클래스로 전환하는 우회도 시도했으나 **"node type is unsupported or does not reference old_class"로 실패**해 self-scope 호출 노드에는 적용 불가함을 실증했다.
**출처**: 2026-08-11 스킬연출구조 E-S0(노드 프로브), [[E-S0_노드프로브_결과]] · §11 함정⑨(원 성공 사례, 이번에 일반화 한계가 밝혀짐).

### 함정 (88) — 노드 검색은 기능명이 아니라 팔레트 카테고리명으로: `InputKey`로 못 찾은 것이 `Keyboard`로는 전부 나왔다
**증상**: `InputKey`로 검색해 "검출 안 됨"으로 판정했던 항목이, **`Keyboard` 카테고리로 검색하니 `Input|KeyboardEvents|A` 형태로 전부 검출**됐다(생성·컴파일 성공까지 확인).
...
## 37. 스킬연출구조 E-S2'(VFX 텍스처 임포트)에서 확정된 노하우 (2026-08-11)

> 배경: 픽셀아트 VFX 3장을 legacy 2장과 동일 세팅으로 임포트하려다 **임포트 기본값이 픽셀아트에 정면으로 어긋난다**는 것을 실측했다. §36에 이어 **(91)부터.**

### 함정 (91) — 텍스처 임포트 기본값은 픽셀아트에 맞지 않다 (임포트만 하면 끝이 아니다)

**증상**: 픽셀아트 스프라이트시트를 임포트했는데 확대 시 뭉개지거나 도트 경계에 색 번짐이 생긴다. 임포트 자체는 에러 없이 성공한다.

**원인**: UE 임포트 기본값이 **`filter=TF_Default`(Linear 보간)** · **`compressionSettings=TC_Default`(블록 압축)**다. 둘 다 픽셀아트에는 파괴적이다 — Linear는 텍셀 경계를 뭉개고, 블록 압축은 하드 픽셀 경계에서 링잉을 만든다. **컴파일 에러도 경고도 안 난다.**

**해법**: 임포트 후 **명시적으로** `TF_Nearest` · `TC_EditorIcon`으로 고친다. 실증(2026-08-11 E-S2'): 3장 전부 기본값이 `TF_Default`/`TC_Default`로 들어왔고, 나머지 8항목(`sRGB=true` · `TMGS_NoMipmaps` · `TEXTUREGROUP_World` · `maxTextureSize=0` · `TA_Wrap` 등)은 기본값부터 legacy와 일치했다.

★**왜 치명적인가**: 이 두 항목이 틀린 채로 룩 판정을 하면 **원인을 엉뚱한 데서 찾는다.** 도트를 확대할 때의 뭉개짐을 *"픽셀 밀도 문제"*로, 압축 링잉을 *"블룸 과다"*로 오독하게 된다 — 증상이 진짜 문제와 구별되지 않는다.

★**GridY>1(색 변형 시트)에서 특히 위험**: `V=(LocalV+RowIndex)/GridY` 수식은 셀 경계 위아래가 **다른 색 행**이다. Linear 필터나 밉이 켜지면 *"시안 이펙트 가장자리에 보라 테두리"*가 생긴다 — GridY=1 시절엔 없던 신규 실패 모드.

**출처**: 2026-08-11 스킬연출구조 E-S2'(VFX 텍스처 임포트).

---

## 38. 스킬연출구조 E-S4a-0(struct 편집 자동화 조사)에서 확정된 노하우 (2026-08-11)

> 배경: *"MCP 토큰셋에 struct 편집 툴이 없다 → 오너 수동뿐"*이라는 결론을 **여러 문서에 반복 기록**해 왔는데, 조사해 보니 **이미 3주 전에 스크립트로 해낸 전례**가 있었다. §37에 이어 **(92)부터.**

### 함정 (92) — "MCP 툴셋에 없다"는 "자동화 불가"가 아니다 (경로가 하나가 아니다)

**증상**: 어떤 작업을 자동화하려다 MCP 툴 목록에 해당 툴이 없는 것을 확인하고 **"오너 수동"으로 결론**낸다. 그 결론이 문서에 박히고, 이후 아무도 다시 의심하지 않는다.

**원인**: **MCP는 자동화 경로의 하나일 뿐인데 전부라고 가정**한 것. 이 프로젝트에는 최소 **세 축**이 있다:

| 축 | 예 | 특징 |
|---|---|---|
| **MCP 툴셋** | `unreal-mcp` · `UmgMcp` | 목록이 유한하고 조회가 쉬워 **여기만 보게 된다** |
| **에디터 Python** | `PythonScriptPlugin` · TAPython `PythonStructLib` | 툴 목록에 안 나온다. **엔진 API 전체에 닿는다** |
| **디스크 직접 조작** | CSV·`.ini`·소스 파일 | 애셋(`.uasset`)엔 안 통하지만 데이터엔 통한다 |

**실증(2026-08-11)**: *"struct 편집은 오너 GUI 수동뿐"*이라는 결론이 D5.5 절차서·plan·기능허브에 기록돼 있었다. 그런데 **2026-07-16에 `f7b_struct_bootstrap.py`가 `F_SkillsRow`를 15→23필드로 실제 확장한 실적**이 있었다(§33에 이미 등재돼 있었다). **"MCP 툴셋에 없다"는 참이었지만 결론이 틀렸다.**

★**왜 이 오판이 오래 살아남는가**: 근거가 **참**이라 검증해도 계속 통과한다. 틀린 건 근거가 아니라 **근거에서 결론으로 가는 도약**이다. "X 툴셋에 없다" → "불가능하다"는 **X가 유일한 경로일 때만** 성립하는데, 그 전제는 한 번도 명시적으로 검사되지 않는다.

**해법 — "불가능" 결론 앞에 붙일 체크**:
1. **어느 경로를 확인했나**를 결론에 함께 적어라. *"MCP 툴셋에 없음"*까지만 적고 *"따라서 불가"*로 넘어가지 마라
2. **다른 축을 최소 하나 확인**해라 — 특히 **Python 경로**. 플러그인 활성 여부는 `.uproject`/`Plugins/` 조회로 즉시 확인된다
3. ★**과거 실적을 검색해라.** 이 프로젝트는 노하우 문서에 전례를 남긴다. *"이 작업을 전에 해낸 적이 있나"*를 먼저 물었으면 §33에서 바로 나왔다

**동류 함정**: (87) *"검색 성공 ≠ 생성 가능"* · (88) *"노드 검색은 카테고리명으로"* · (79) *"검색 0건 = 없다 오판"*. 넷 다 **한 축에서 얻은 부정 결과를 전역 부정으로 일반화**한 것이다. 상위 원칙: ★**부정 결론은 긍정 결론보다 검증 부담이 크다** — "있다"는 하나만 찾으면 되지만 "없다"는 전 축을 훑어야 한다.

### 함정 (93) — 재사용할 스크립트의 멱등 가드는 **범위**를 먼저 본다

**증상**: 과거에 성공한 스크립트를 다시 돌렸는데 **아무 일도 일어나지 않는다.** 에러도 없다.

**원인**: 멱등 가드가 **작업 단위가 아니라 스크립트 단위**로 걸려 있다. `f7b_struct_bootstrap.py`의 `already_bootstrapped()`는 **`F_StatusEffectRow.uasset` 존재 여부 하나로 `main()` 전체를 SKIP**한다. 그 애셋은 이미 있으므로, 나머지 작업이 미완이어도 영원히 실행되지 않는다.

**해법**: 재사용 전에 가드의 **판정 대상**을 확인한다. 그리고 새로 쓸 때는 **작업 단위 멱등**으로 만든다 — *"이 애셋이 있는가"*가 아니라 *"이 필드가 이미 있는가"*. 그래야 **6개 중 3개만 처리된 중간 실패 상태에서 재실행하면 나머지 3개만** 처리된다.

★**부수 판정 — 고칠 것인가 새로 쓸 것인가**: 이번엔 **새로 썼다.** 그 스크립트가 세 가지 일을 한 `main()`에 묶고 있어, 가드를 고치려면 **이번에 필요 없는, 이미 라이브인 struct 2종의 재실행 안전성까지 판단**해야 했다. 재사용 가치는 **가드가 아니라 검증된 API 호출 패턴**이었으므로 그것만 옮겼다. → **"과거 스크립트 재사용"은 파일 재사용이 아니라 지식 재사용일 때가 많다.**

### 함정 (94) — 스크립트의 안전장치를 **오너의 표준 절차**가 무효화할 수 있다

**증상**: 스크립트가 손상을 감지하고 *"저장하지 않고 중단"*했는데, 결과적으로 **손상이 디스크에 저장된다.**

**원인**: 스크립트는 **자기 프로세스 안**에서만 저장을 막는다. 그런데 `D5.5_오너_struct세션_절차서` 공통주의 ④는 오너에게 **"저장은 Ctrl+S, 그리고 Content Browser에서 Save All"**을 지시한다. 오너가 절차대로 하면 **손상된 in-memory 상태가 그대로 쓰인다.**

**해법**: 자동화의 실패 배너에 **오너 행동까지 포함한 지시**를 넣는다 — *"저장하지 마라(Ctrl+S/Save All 금지) → 저장 없이 종료 → `git checkout` → 보고"*.

★**일반화**: **자동화의 안전 경계는 프로세스가 아니라 사람까지다.** 사람이 이어서 하는 표준 절차가 있으면, 그 절차가 안전장치를 되돌리지 않는지 확인해야 한다. 실패 경로 설계 시 *"이 다음에 사람이 무엇을 하도록 지시받고 있나"*를 물어라.

**출처**: 2026-08-11 스킬연출구조 E-S4a-0(struct 편집 자동화 조사) + E-S4a(스크립트 작성·Director 검토).

---

## 39. 스킬연출구조 E-S2(FxLabQuad)에서 확정된 노하우 (2026-08-11)

> 배경: 에이전트가 *"이 MCP 환경에서 World Tick 자체가 진행되지 않는다 → 타이밍 기반 BP 전체가 구조적으로 검증 불가"*라고 보고. Director가 엔진 로그를 직접 조사해 **진단이 틀렸고 원인이 3주 전에 이 문서에 적어둔 설정**임을 확정했다. §38에 이어 **(95)부터.**

### 함정 (95) — ★함정⑦은 "가능성"이 아니라 **현재 켜져 있는 상태**다 (`max tick rate 3`)

**증상**: `RetriggerableDelay`가 완료되지 않는다. `AsyncLoadAsset`의 `Completed`가 오지 않는다. `Event Tick` 로그가 0줄로 보인다. → *"틱이 안 돈다"*는 결론에 도달한다.

**진짜 원인**: **`bThrottleCPUWhenNotForeground`(에디터 Preferences의 "Use Less CPU when in Background")가 엔진 기본값 `true`로 켜져 있다.** 에디터 창이 포그라운드가 아니면 PIE가 **초당 3틱**으로 제한된다.

**★결정적 판별 — 로그 한 줄**:
```
LogWorld: Bringing World .../<맵> up for play (max tick rate 3)
                                              ^^^^^^^^^^^^^^^^^^
```
PIE 시작 시 **항상** 찍힌다. `3`이면 스로틀, `60`이면 정상.

**★★가장 아픈 부분 — 이 문서에 이미 적혀 있었는데 적용되지 않았다**:
- 함정⑦: *"`bThrottleCPUWhenNotForeground=true`(기본값)이면 PIE 게임 틱이 사실상 정지할 수 있다"*
- §19: *"스로틀 설정 — 개발 기간 동안 **false 유지 권장**"*

그런데 실측하니 **설정 항목 자체가 어디에도 없었다**(`Config/*.ini`·`Saved/Config/**` 전수 grep 0건). 항목이 없으면 기본값 `true`다. → **"권장"이 문서에만 남고 실제로 적용된 적이 없다.**

★**"권장"으로 적은 설정은 적용 여부를 검증할 수단이 없으면 적용되지 않은 것과 같다.** 더 나쁜 건, *"false 유지 권장"*이라는 문장이 다음 사람에게 **"이미 false겠지"라고 믿게 만든다** — 없는 것보다 해롭다. 설정 항목은 **권장이 아니라 실측 명령과 기대 출력**으로 적어야 한다.
...
```
★**`save_config()`가 핵심** — 빼면 이번 세션만 적용되고 재시작 시 원상복귀한다. 지금 상태가 정확히 "저장된 적 없음"이다.
**확인**: PIE를 켜고 로그에 `max tick rate 60`이 나오는지 본다.

**왜 3fps에서 `AsyncLoadAsset`이 실패하는가**: 게임 시간(델타타임)은 3fps에서도 정상 누적되므로 `Delay(0.45)`는 원리적으로 2틱이면 끝난다. 그러나 **비동기 에셋 로딩은 틱당 시간 예산에 묶여** 있어 초당 3틱이면 스트리밍이 사실상 진행되지 않는다. `Completed`가 안 오고, `then` 핀으로 우회하면 그 시점엔 `Object`가 `None`이라 캐스트 실패로 빠진다 — **"텍스처 경로가 틀렸다"로 오독하기 딱 좋다**(실제로 텍스처는 디스크에 있었다).

### 함정 (96) — "틱 정지"와 "틱 스로틀"은 증상이 같고 원인이 다르다 — **프레임 카운터로 가른다**

엔진 로그 대괄호 **두 번째 값이 프레임 번호**다: `[2026.08.11-07.24.55:157][768]`

| 관측 | 해석 | 대응 |
|---|---|---|
| 프레임 번호 **고정** | 진짜 정지 — 블로킹 모달(함정㉓) 계열 | 오너 육안 + 모달 닫기 |
| 프레임 번호 **느리게 증가** | 스로틀 | (95) 해법 |
| 같은 로그에 **정상 세션이 섞여 있음** | ★**구조적 불가능이 아니다** — 조건 의존 | 조건 차이를 찾아라 |

**실증(2026-08-11)**: 프레임이 `768 → 973`(68초에 205프레임 ≈ 3.0/초)으로 **진행**했고, 같은 로그에 `max tick rate 60` 세션이 **2회** 섞여 있었다(14:2). 둘 다 *"틱이 진행되지 않는다"*를 반증한다.

★**에이전트가 `Event Tick`에 로그를 걸어 확인한 것이 왜 오도했나**: 3fps에서는 로그 버퍼 조회 타이밍에 따라 **0줄로 보일 수 있다.** 반면 `max tick rate`는 PIE 시작 시 **결정론적으로 한 줄** 찍힌다. **관측 수단을 고를 때 "내가 만든 신호"보다 "엔진이 항상 남기는 신호"를 우선하라.**

### 함정 (97) — 부정적 결론의 **크기**에 비례해 검증을 늘려라

에이전트의 결론은 *"타이밍 기반 BP 로직 전체를 이 MCP 경로로는 검증할 수 없다"*였다 — **프로젝트의 검증 수단 하나를 통째로 폐기하는 크기**다. 그런데 근거는 **자기가 만든 BP 하나의 관측**이었다.

3회 실패 규칙에 따라 중단하고 보고한 것은 **옳다.** 문제는 중단 지점이 아니라 **결론의 크기**다. 5회 시도가 전부 같은 축(warmup 늘리기·PlayMode 바꾸기·핀 스왑)이었고, **"내 BP 밖"을 보는 시도가 없었다** — 엔진 로그 한 줄이면 끝날 일이었다.

★**규칙**: 결론이 *"이 도구/경로는 원리적으로 안 된다"* 급이면, 보고 전에 최소 하나는 **자기 산출물 바깥의 증거**를 대라 — 엔진 로그, 과거 성공 기록, 같은 도구의 다른 사용례. 없으면 결론을 *"내 케이스에서 재현되지 않았다"*로 낮춰 보고한다. (87)(92)와 같은 계열 — **부정 결론은 긍정 결론보다 검증 부담이 크다.**

**출처**: 2026-08-11 스킬연출구조 E-S2([[E-S2_틱스로틀_진단]]).

---

## 40. §39 함정(95) 자기 정정 + 신규 경로 발견 (2026-08-12)

### ★함정 (95) 정정 — 원인 귀속이 틀렸다. **설정은 원래부터 `false`였다**

§39에서 *"`bThrottleCPUWhenNotForeground`가 기본값 `true`로 켜져 있고, 문서의 'false 유지 권장'이 실제로 적용된 적이 없다"*고 썼다. **틀렸다.**

**실측(2026-08-12)**: `ConfigSettingsToolset.GetSectionPropertyValues`로 런타임 값을 직접 읽으니 **`false`**였다. ini에 항목이 없는 것은 *"적용 안 됨"*이 아니라 **엔진 기본값이 이미 `false`**라는 뜻이었다.

~~"권장이 문서에만 남고 적용된 적이 없다"~~ → **설정은 정상. 원인은 다른 곳.**

**그럼 `max tick rate 3`은 왜 나왔나 — 실제 이력이 답을 준다**:

| 시각 | tick rate | 누가 PIE를 켰나 |
|---|---|---|
| ~07:36 | **3** | 에이전트 |
| 11:06 · 12:14 · 14:23 | **60** | 오너 |
| 14:37 | **3** | 에이전트 |
| 14:53 · 15:01 | **60** | 오너 |

→ ★**설정과 무관하게, 에디터 창이 백그라운드면 3으로 떨어진다.** 오너가 직접 PIE를 켜면(포그라운드) 60, **에이전트가 켜면 오너가 다른 창을 보고 있어 3**이다.

**실무 함의 — 이쪽이 원래 함정보다 중요하다**:
- **에이전트의 PIE 검증은 구조적으로 3fps에서 이뤄진다.** 오너가 화면을 지켜보고 있지 않는 한 그렇다
- 그래서 **타이밍 기반 검증(Delay·비동기 로딩·애니메이션 프레임)은 에이전트 PIE로는 신뢰할 수 없다**
- **대응**: PIE 검증 보고에 `max tick rate` 값을 **반드시 함께 적는다.** `3`이면 정적 판정(위치·값·컴파일)만 신뢰하고 **타이밍 판정은 오너 육안으로 넘긴다**

★**정정이 남긴 교훈은 (92)의 재발이다**: *"ini에 항목이 없다 → 기본값 `true`겠지"* — **한 축(파일)만 보고 다른 축(런타임 객체)을 안 봤다.** 바로 앞 섹션에 그 함정을 써놓고 같은 실수를 했다. **부정 결론뿐 아니라 "기본값이 무엇인가" 같은 단순 사실도 읽을 수 있으면 읽어라.**

**(96)(97)은 유효**하다 — 프레임 카운터로 정지/스로틀을 가르는 법, 부정 결론의 검증 부담. 진단 방법은 맞았고 원인 귀속만 틀렸다.

### 함정 (98) — 에디터 Preferences는 **MCP로 직접 읽고 쓸 수 있다** (`ConfigSettingsToolset`)

지금까지 *"에디터 설정 변경 = 오너가 Preferences에서 눌러야 한다"*로 취급해 왔다. **틀렸다.** `ConfigSettingsToolset`이 있다:

```
ListContainers / ListCategories / ListSections     ← 탐색
GetSectionSchema / GetSectionPropertyValues        ← 읽기
SetSectionProperties / SaveSection                 ← 쓰기·저장
ResetSectionToDefaults                             ← 되돌리기
```

실증: `Editor → General → EditorPerformanceSettings`에서 `bThrottleCPUWhenNotForeground` 값을 읽어 (95)의 오진을 잡았다.

**호출 형식 주의**: `call_tool(tool_name="ListCategories", toolset_name="ConfigSettingsToolset.ConfigSettingsToolset", ...)` — **툴 이름에 툴셋 접두어를 붙이면 "not found"**가 난다. 접두어는 `toolset_name`에만.

★**이것도 (92)의 사례다** — *"오너가 GUI에서 해야 한다"*는 결론이 **툴셋 목록을 끝까지 안 본** 결과였다. **`list_toolsets`는 45개 툴셋을 반환하는데 우리가 습관적으로 쓰는 건 그중 대여섯 개다.** 막히면 목록을 다시 훑어라.

**출처**: 2026-08-12 E-S2 후속 진단([[E-S2_틱스로틀_진단]]).

---

## 41. 라이브 전투 자동 검증의 벽 — 원인 특정 (2026-08-12)

> 배경: T1(라이브 BP 수술) 실증을 verifier에게 맡겼는데 **전투를 한 턴도 진행시키지 못했다.** 그 원인을 컴포넌트 단위까지 특정했다. §40에 이어 **(99)부터.**

### 함정 (99) — 에이전트 PIE로는 **클릭 기반 전투를 진행시킬 수 없다.** 4경로 전부 봉쇄

| 경로 | 결과 | 원인 |
|---|---|---|
| `SlateInspectorToolset` | ❌ | `Windows(list)` → `[]`. 클릭 대상 위젯 레퍼런스가 없다 |
| `EditorAppToolset` | ❌ | **게임 월드 입력 주입 툴 자체가 없다.** `FocusOnActors`는 *"PIE 활성 중 호출 불가"* 명시 차단 |
| **BP 함수 직접 호출** | ❌ | ★핵심 — 트리거가 `OnClicked(ClickBox)`, **PrimitiveComponent에 바인딩된 네이티브 델리게이트**다. 엔진 HitProxy 파이프라인으로만 발화하고 **호출 가능한 UFunction이 아니다** |
| 콘솔 명령 | ❌ | 전 툴셋에 콘솔 실행 툴 없음(`SearchCVars`는 조회 전용) |

★**막힌 지점은 정확히 1개**다 — "Start" 클릭 **단 1회**. 그 뒤 상태머신(`TurnStart → AwaitCommand → … → TurnEnd`)은 자동 진행된다. verifier 세션은 `State:Init:t=0`에서 수 분간 정지했다.

**해법(설계 권고)**: 우회하려 애쓰지 말고 **디버그 훅을 심어라** — 콘솔 명령 훅 / 자동 시작 토글 / 디버그 전용 BP 함수. **한 번 넣으면 이후 모든 라이브 검증이 자율화**된다. 지금은 매 검증마다 오너 클릭 1회가 선행 조건이다.

### 함정 (100) — §39~40의 tick rate 문제, **최종 확정**

두 번 정정한 끝에 확정한다:

| 판본 | 주장 | 판정 |
|---|---|---|
| §39 (95) | *"`bThrottleCPUWhenNotForeground` 기본값 `true`가 적용 중"* | ❌ **틀림** — 설정은 원래부터 `false` |
| §40 정정 | *"설정과 무관하게 창이 백그라운드면 3"* | ✅ 방향은 맞음 |
| **§41 확정** | ★**사람이 앞에 있으면 60, 에이전트가 돌리면 3.** 에이전트는 이를 바꿀 수 없다 | **확정** |

**실측 근거**: 오너가 직접 켠 PIE **5회 전부 60** / 에이전트 세션 **전부 3**. 그리고 에이전트 세션에서 `SlateInspector`는 창을 **0개**로 보고 `CaptureEditorImage`는 실패하는데, **`CaptureViewport`(렌더타깃 경유)는 정상 동작**한다.

★**"OS 창이 0개"로 단정하지 마라** — 오너가 에디터 스크린샷을 보낸 사실이 반증이다. 창은 존재하되 **비활성**이고 Slate 열거 API가 그 상태를 못 잡는 것으로 보인다. **원인 해석은 미확정이지만 실무 결론은 같다.**

**실무 규칙 3개**:
1. **PIE 검증 보고에 `max tick rate`를 반드시 적는다**
2. **`3`이면 정적 판정(핀 원문·컴파일·프로퍼티 값·로그 발화 유무)만 신뢰**하고, 타이밍·프레임 성격 판정은 **오너 육안으로 이월**한다(사유·단계·승인자 기록)
3. ★**"3fps를 우회해 보겠다"는 시도를 반복하지 마라.** 두 번 조사해 두 번 다 불가였다. 답은 우회가 아니라 **오너 세션 또는 디버그 훅**이다

### 함정 (101) — 로그에 남은 **과거 전투 흔적**을 현재 검증으로 착각하지 마라

verifier 세션 로그에 `TakeHit` · `BattleLog|dmg=32`가 남아 있었다. 그대로 *"피격 발화 확인 = PASS"*로 처리할 수 있었다.

**타임스탬프 대조**로 그것이 **수술 전 베이스라인(14:33:28) ~ 최종 저장(16:18:06) 사이**의 데이터임이 드러나 배제됐다.

★**엔진 로그는 세션 경계를 지우지 않는다.** 수술 전후를 가르려면 **`SavePackage` 로그 시각과 대조**하라. 이 프로젝트가 반복해온 오판(**과거 관측 → 현재 검증 착각**)의 로그 버전이고, 이번엔 검증자가 스스로 막았다.

**출처**: 2026-08-12 A1 T1 실증([[A1_T1검증_및_트리거벽]]).

---

## 42. AT4-a(FxLab Director 구현)에서 확정된 노하우 (2026-08-12)

> 배경: `BP_FxLabDummy` 신설 + FxLab Director 내장 세션. §41에 이어 **(102)부터.**

### 함정 (102) — ★`save_assets(asset_paths=[])`(전체 dirty 저장)가 "라이브 BP 무접촉" 규율을 무음으로 위반시킬 수 있다 — 조회만으로 dirty된 무관 애셋까지 함께 저장됨

**증상**: 신규 BP(`BP_FxLabDummy`) 작업을 마치고 관례대로 `save_assets(asset_paths=[])`(빈 배열=전체 dirty 저장, §1 line54가 권장하는 패턴)를 호출했다. 세션 종료 후 `git status`를 보니 의도한 파일(신규 BP + 레벨) 외에 **`BP_BattleManager.uasset`·`BP_FxLabQuad.uasset`·`WBP_SkillMenu.uasset`**까지 `M`(수정됨)으로 잡혔다 — 이번 세션에서 **셋 다 편집한 적이 없다**(`BP_BattleManager`는 지시서가 명시적으로 무접촉을 요구한 대상).

**원인**: E-S0([[E-S0_노드프로브_결과]] "부수 확인")가 이미 *"순수 조회만으로 BP가 `is_dirty=true`가 되는 현상"*을 관측했었다 — 그때는 `save_assets` 미호출이라 디스크에 반영되지 않아 "무해"로 처리됐다. 이번엔 그 잠재적으로 dirty해진 애셋들이 있는 상태에서 **`save_assets([])`를 호출**했고, 이 API는 이름 그대로 **프로젝트 전체의 dirty 애셋을 무차별로 저장**한다 — 두 개별 사실(①조회만으로 dirty됨 ②`save_assets([])`는 전체 저장)이 만나 **"관련 없는 애셋까지 디스크에 재기록"**이라는 새 위험을 만든다. 실제 diff는 파일당 2~4줄(binary diff 기준) 수준 — 로직 변경이 아니라 **재직렬화 시점 차이로 추정**되는 메타데이터성 변경으로 보이나, **"라이브 BP 무접촉"이 하드 룰인 프로젝트에서는 이 정도도 규율 위반**이다.

★**되돌리기 시도도 막혔다**: `git checkout --`/`git restore` 둘 다 이 환경의 권한 분류기가 차단했다(파괴적 git 조작으로 분류). 에이전트 권한으로는 **원복 자체가 불가능**했다 — 결국 커밋을 의도한 파일만 `git add`로 골라 스테이징하고, 무관 변경분은 **워킹트리에 미커밋 상태로 남긴 채** PM/오너에게 명시 보고하는 것이 유일한 안전 대응이었다.

**해법**:
1. **"라이브 무접촉" 대상이 있는 프로젝트에서는 `save_assets(asset_paths=[])`를 쓰지 마라.** 항상 **의도한 애셋의 명시적 경로 리스트**를 넘겨라(`save_assets(["/Game/Blueprints/BP_FxLabDummy"])`처럼). §1의 "실험 후 전체 dirty 저장" 권장은 **무접촉 제약이 없는 스파이크/실험용 세션에 한정**해서 읽어야 한다.
2. 세션 종료 전 **`git status`(Content 저장소)로 의도치 않은 변경 파일이 있는지 반드시 확인**하고, 있으면 `git add`에 **의도한 파일만 명시적으로 나열**해 스테이징 — `git add -A`/`git add .` 금지.
3. 에이전트는 `git checkout`/`git restore` 권한이 없을 수 있다 — 되돌리기가 필요하면 **직접 시도하지 말고 PM/오너에게 위임**할 것(무단 우회 시도 금지, 이 문서 원칙과 별개로 실행 환경의 권한 경계이기도 하다).

**출처**: 2026-08-12 스킬연출구조 AT4-a([[AT4-a_결과_2026-08-12]]).

### 함정 (103) — `get_node_type_pins`로 프로브한 노드가 **자동 GC(자체 삭제)되는 사례를 재확정** — `ProgrammaticToolset` 실패 스크립트는 노드 ID 카운터를 롤백하지 않는다

**배경**: §7 함정①이 이미 *"`get_node_type_pins`가 그래프에 임시 노드를 만들었다가 나중에 조용히 GC된다"*를 문서화해 두었다 — 이번 세션은 **그 함정을 읽지 않고 직접 재현**했다(교훈: 함정①을 세션 시작 전에 먼저 훑을 것). 재현 사실만 덧붙인다: `get_node_type_pins`로 self-call 함수(`PlayApproach` 등)의 파라미터 순서를 확인하려 만든 프로브 노드(`K2Node_CallFunction_2/3/4`)를, **같은 그래프의 후속 `ProgrammaticToolset` 스크립트에서 `connect_pins`로 재사용**했다가 컴파일이 `Internal compiler error inside CreateExecutionSchedule`로 실패했다 — 참조한 노드가 이미 사라진 상태였다.

★**신규 관찰(함정①에 없던 부분)**: 이 실패한 스크립트가 **롤백된 뒤에도, 다음 성공한 스크립트가 만드는 새 노드의 자동 번호(`K2Node_CallFunction_N`)는 실패 이전 시점부터 계속 증가**했다(예: 실패 스크립트가 `_8`까지 만들고 롤백됐는데, 다음 성공 스크립트의 첫 노드가 `_9`가 아니라 그 이후 번호로 시작). 즉 **"내용은 롤백되지만 이름 카운터는 롤백되지 않는다"** — 다음 스크립트에서 이름을 **예측(하드코딩)하지 말고, 항상 직전 호출의 반환값에서 실제 `refPath`를 받아 재사용**해야 한다(함정(56)이 이미 스크립트 트랜잭션 롤백 자체는 다뤘으나, 이름 카운터의 비롤백은 별개 사실).

**해법**: (1) `get_node_type_pins`는 스키마 확인 전용으로만 쓰고, 그래프에 실제로 심을 노드는 **항상 `create_node`로 별도 생성**(함정①과 동일). (2) `ProgrammaticToolset` 스크립트가 **여러 단계에 걸쳐 같은 그래프를 확장**할 때는 다음 단계 스크립트를 짜기 전에 **`get_connected_subgraph`/`find_nodes`로 현재 실제 상태를 재조회**해 노드 refPath를 확정하고, 이전 단계의 반환값이나 이름 패턴을 추측해 하드코딩하지 말 것.

**출처**: 2026-08-12 스킬연출구조 AT4-a — `ResolveStaging` 함수 구현 중 `Internal compiler error` 재현 및 원인 규명([[AT4-a_결과_2026-08-12]]).

### 함정 (104) — 에디터 로캘이 노드 검색 문자열에 미치는 영향 3사례 추가 확정 — `유틸리티|연산자|<한글>(<기호>)`(제네릭 와일드카드 연산자) · `Variables|디폴트|<Name>`(멤버변수) · `Utilities|데이터테이블행가져오기`(GetDataTableRow 와일드카드)

함정(79)·(88)이 이미 "노드 검색은 기능명이 아니라 팔레트 카테고리명으로, 이 프로젝트 로캘은 한국어"를 확정해 두었다. 이번 세션에서 **구체적으로 막혔던 3개 패턴**을 실측값으로 남긴다(다음 사람이 같은 시행착오를 반복하지 않도록):

| 찾던 것 | 안 되는 시도 | 되는 검색어(`find_node_types type_id_filter`) | 비고 |
|---|---|---|---|
| 정수/실수 `!=` | `수학|인티저|integer!=integer` | `같지않음` → `유틸리티|연산자|같지않음(!=)` | **와일드카드 A/B 핀** — 연결 후 자동 승격(함정(80) 패턴과 동일) |
| `<=` | `수학|플로트|float<=float` | `이하` → `유틸리티|연산자|작거나같음(<=)` | 〃 |
| 벡터/실수 `+` | `Transformation|...`류 영문, `더하기` | `추가` → `유틸리티|연산자|추가` | 〃. **"더하기"로는 0건** — "추가"만 맞다 |
| 신규 멤버변수 Get/Set | `Variables|Default|Get<Name>`(DSL 문서의 영문 예시 그대로) | `Variables|디폴트|Get<Name>`/`Set<Name>` | DSL 문서(`get_graph_dsl_docs`)의 영문 예시는 **이 프로젝트 로캘에 안 맞는다** — 로캘 무관 문서 예시를 그대로 믿지 말 것 |
| `GetActorLocation`/`SetActorLocation`/`SetWorldRotation` | `Transformation|GetActorLocation`(영문) | `트랜스포메이션|GetActorLocation`(카테고리만 한글, 함수명은 영문 유지) | 카테고리·함수명이 **혼합 로캘**인 케이스 — 전부 영문도, 전부 한글도 아니다 |
| `GetDataTableRow`(와일드카드, 임의 DataTable 대응) | `Utilities|GetDataTableRow`(빈 결과) · `Utilities|GetDataTableRowDT_Stagings`(생성 시점엔 "does not exist") | `데이터테이블` 카테고리 전체를 나열해서 찾음 → `Utilities|데이터테이블행가져오기` | **생성 시엔 이 문자열로**, `DataTable` 입력핀에 리터럴 경로를 `set_pin_value`하면 **`ReturnValue`가 와일드카드→해당 테이블의 Row Struct로 자동 해석되고 노드의 표시 `type_id`도 사후적으로 `Utilities|GetDataTableRowDT_<TableName>` 형태로 바뀐다**(BP_FxLabQuad의 기존 노드가 후자로 보였던 이유 — **생성 시 이름과 조회 시 이름이 다르다**는 게 핵심 함정) |

★**공통 대응**: 특정 함수/기호로 검색해 0건이면 **포기하지 말고 상위 카테고리(한글) 전체를 `type_id_filter`에 넣어 나열**한 뒤 눈으로 스캔할 것 — 이번 세션 5건 전부 이 방법으로 풀렸다.

**출처**: 2026-08-12 스킬연출구조 AT4-a([[AT4-a_결과_2026-08-12]]).

### 함정 (105) — 함정(100) 3차 재확인 + 신규 설정 1건 추가 — `bAllowSlateThrottling`도 에이전트-PIE의 `max tick rate 3`을 못 고친다

함정(100)이 이미 *"사람이 PIE를 켜면 60, 에이전트가 켜면 3 — 설정과 무관하며 에이전트는 이를 바꿀 수 없다"*로 확정하고 *"3fps를 우회해 보겠다는 시도를 반복하지 마라"*로 규범화했다. 이번 세션은 **지시서가 명시적으로 재시도를 요청**했기에(오너/PM이 "혹시나" 확인을 원함) 규칙과 상충하지만 3차 시도를 실행했다.

**신규 정보 1건**: `EditorPerformanceSettings`엔 `bThrottleCPUWhenNotForeground`(§39~40이 이미 조사) 외에 **`bAllowSlateThrottling`**("UI 인터랙션 동안 백그라운드 처리[월드 틱·뷰포트]를 제한 — 비활성화하면 인터랙션 중에도 뷰포트·시뮬레이션이 최대 속도 유지")이라는 **별개 설정**이 있다. 이름과 설명만 보면 에이전트의 MCP 자동화(=일종의 "UI 인터랙션")가 걸릴 후보로 보였다. `false`로 전환(`SetSectionProperties`+`SaveSection`, 재조회로 적용 확인)했으나 **tick rate는 그대로 3**이었다.

★**결론**: 함정(100)의 확정이 다시 한번 맞았다 — **설정 축은 이걸로 사실상 소진됐다.** 다음 사람은 `bThrottleCPUWhenNotForeground`·`bAllowSlateThrottling` 둘 다 이미 헛수고였음을 알고 **설정을 더 뒤지지 말고 곧장 "오너 세션 또는 디버그 훅"으로 가라**(함정(100) 실무 규칙 3 그대로 재확인).

**출처**: 2026-08-12 스킬연출구조 AT4-a([[AT4-a_결과_2026-08-12]]).

---

## 43. DT_Vfx texPath 서픽스 보정 + AU-A1-09 실측대조에서 확정된 노하우 (2026-08-12)

> 배경: `PlayVfx` `FXSHOW` 0건 미스터리의 근본원인 규명(AT4-b-2) + `DT_Vfx.texPath` 서픽스 보정·검증(DT_Vfx보정) + T1 전후 턴길이 실측대조(AU-A1-09). §42에 이어 **(106)부터.**

### 함정 (106) — ★★소프트 오브젝트 경로 문자열에 `.AssetName` 서픽스가 없으면 `MakeSoftObjectPath`→`AsyncLoadAsset`→`Cast`가 조용히 실패한다(파일이 실재해도)

**증상**: `DT_Vfx`의 `texPath`(문자열 컬럼, 예: `"/Game/VFX/T_FX_Smear"`)를 `MakeSoftObjectPath`→`ToSoftObjectReference`→`AsyncLoadAsset`→`CastToTexture2D`로 로드하는 `PlayVfxInternal` 체인에서, **텍스처 파일이 실재함에도**(`AssetTools.exists=true`) `CastToTexture2D`가 매번 `CastFailed` 분기로 빠져 `FXLAB:TEXMISS`만 발화했다. 배선 자체는 여러 차례 재확인해도 멀쩡했고, 이 상태가 **3주 넘게(AT4-pre~AT4-b 다단계 조사)** 원인 미확정으로 남아 있었다.

**원인**: `"/Game/VFX/T_FX_Smear"`는 **패키지 경로**일 뿐이다. `FSoftObjectPath`가 실제 오브젝트를 리졸브하려면 `"/Game/VFX/T_FX_Smear.T_FX_Smear"`처럼 **`Package.AssetName` 형식의 `.AssetName` 서픽스**가 필요하다. 같은 그래프의 하드 레퍼런스(`CreateDynamicMaterialInstance`의 `SourceMaterial` 핀)는 에디터가 자동으로 완전한 경로(`.MI_FX_Smear` 서픽스 포함)를 채워주지만, `DT_Vfx.texPath`는 **DataTable 문자열 컬럼에 사람이 손으로 입력**한 값이라 서픽스가 누락돼 있었다.

**★통제 실험(실측)**: `DT_Vfx` 5행 중 `63000100`만 `texPath`를 `"/Game/VFX/T_FX_Smear.T_FX_Smear"`로 임시 수정 → **같은 PIE 실행에서** `FXLAB:FXSHOW:63000100:t=3.666668` 실발화(3인스턴스 전부, 동일 프레임). **동일 실행에서** 서픽스 미수정 `63000300`은 여전히 `FXLAB:TEXMISS:63000300:...`. 원복 후 `get_rows` 재조회로 바이트 단위 일치 확인. 이후 5행 전부 서픽스를 정식 보정하고 별도 세션에서 재검증 → **5/5 `FXSHOW` 실측 PASS**(`FXNOROW` 등 기존 fail-loud 회귀 없음).

**★왜 3주 넘게 안 잡혔나**: 코드(배선)는 처음부터 멀쩡했다 — `AsyncLoadAsset`의 `Completed`에 `CastToTexture2D`가 정확히 물려 있음을 수 차례 재확인했다. 조사가 반복적으로 **틱레이트(3fps)·배선·리터럴**만 의심했고 **데이터 값 자체**는 의심하지 않았다. `TEXMISS` fail-loud 로그가 실패를 잡아내긴 했지만 *"캐스트 실패"*까지만 말했지 **어떤 값으로 실패했는지**(=경로 문자열 원문)를 로그에 남기지 않아, 값을 눈으로 보고 서픽스 누락을 알아챌 기회 자체가 없었다.

**규칙**:
1. ★소프트 오브젝트 경로를 문자열 컬럼(DataTable 등)에 저장할 때는 반드시 **`.AssetName` 서픽스를 포함**한 완전한 경로로 입력한다.
2. 입력 전 `AssetTools.exists` + `get_asset_class`로 **경로 실재·타입을 확인**하되, 이것만으로는 서픽스 누락을 못 잡는다(패키지 경로만으로도 `exists=true`가 나온다) — 별도로 **서픽스 포함 여부를 육안/정규식으로 점검**할 것.
3. ★fail-loud 로그는 **"무엇이 실패했는가"뿐 아니라 "무슨 값으로 실패했는가"까지 남겨라** — `TEXMISS:<id>:<경로원문>` 형태였으면 이 함정은 첫날 잡혔을 것.

**★아직 안 터진 곳(예방)**: `DT_Sfx.soundPath`가 **완전히 동일한 패턴**(소프트 경로를 raw string 컬럼에 저장)이다. 현재 3행 전부 `"NONE"` sentinel이라 결함이 **휴면 상태**이고, 오디오 데이터가 실제로 채워지는 순간(D6/오디오 임포트 단계) 서픽스 없이 손입력되면 100% 재발이 예상된다. `DT_Skills`의 `castFX`/`projectileFX`/`impactFX`/`castSFX`/`impactSFX` 5개 문자열 컬럼도 동일 패턴이지만 전 행 공백(실사용은 병렬 정수 ID 컬럼) — 죽은 슬롯으로 추정되나 balance-designer 의도 확인 필요.

**출처**: 2026-08-12 스킬연출구조 AT4-b-2(원인 규명, [[AT4-b-2_결과_2026-08-12]]) + DT_Vfx.texPath보정(수정·검증, [[DT_Vfx_texPath보정_2026-08-12]]).

### 함정 (107) — `BlueprintTools.connect_pins`는 이미 배선된 출력 exec 핀에 새로 연결하면 **기존 연결을 무음으로 교체**한다 — §6(line129)의 "exec fan-out 불가" 서술을 실전 사고로 보강

§6이 이미 "exec 출력 핀은 fan-out 불가(데이터 핀만 가능)"라는 UE 일반 제약을 적어뒀으나, 이번 세션은 **그 제약이 MCP `connect_pins` 사용 중 실제로 어떤 사고를 내는지**를 실측으로 보강한다.

**증상**: `BP_FxLabQuad`에서 진단용 트리거를 걸기 위해 `connect_pins`로 `BeginPlay.then`을 새 테스트 노드에 바로 이었더니, **기존 초기화 체인(`CreateDynamicMaterialInstance→SetMid→SetMaterial→SetVisibility→SetCollisionEnabled`)이 끊겼다** — 에러도 경고도 없었다. 그대로 뒀다면 `Mid`가 `None`인 채 `PlayVfx`가 실행되어 **원인이 뒤섞인 거짓 진단 결과**가 나올 뻔했다.

**원인**: UE Blueprint의 exec 출력 핀은 원래 1개 연결만 가질 수 있다(§6). `connect_pins`는 이 제약을 그대로 따르되, **새 연결을 만들 때 기존 연결을 지워도 되는지 확인하거나 경고하지 않고 조용히 교체(덮어쓰기)한다** — "추가 연결"로 오인하기 쉬운 지점.

**규칙**:
1. 배선 전 **대상 출력 exec 핀이 미배선 상태인지 `get_node_infos`로 먼저 확인**한다.
2. 이미 배선돼 있다면 직접 잇지 말고 ★**`Sequence` 노드를 먼저 심고** 원래 연결은 `Then_0`으로, 새 연결은 `Then_1`(등)으로 분기시킨다 — 이건 additive라 기존 exec 체인이 끊기지 않는다.
3. ★배선(또는 재배선) 후에는 **반드시 `get_node_infos`로 재조회**해 무음으로 끊긴 연결이 없는지 확인한다.
4. ★**입력 exec 핀도 같은 성질(덮어쓰기)을 갖는지는 이번 세션에서 미확인**이다 — 출력 핀만 실측 확인됨. 입력 핀 배선 시엔 보수적으로 동일 절차(사전 확인+사후 재조회)를 적용할 것.

**출처**: 2026-08-12 스킬연출구조 AT4-b-2([[AT4-b-2_결과_2026-08-12]]) §"트리거 배선(임시)".

### 함정 (108) — ★UE `float→string`은 로케일 포맷을 타 천 단위 쉼표가 붙는다(`int→string`은 안 붙는다) — 1000 이상에서만 발현, 짧은 세션에선 영원히 안 보인다

**증상**: PIE 로그 파싱 스크립트(정규식 `[\d.]+`)로 턴 길이를 델타 계산하던 중, 특정 세션(21:14)만 이질적인 결과(`0.026`처럼 비정상적으로 작은 델타)가 나왔다. 원문을 끝까지 보지 않고 "조건이 다른 세션"으로 배제했다면, *"턴 2부터 타임스탬프가 `6`에 고정되는 로그 결함"*이라는 **실재하지 않는 결함**을 결함으로 기록할 뻔했다.

**원인**: 로그 원문은 `State:AwaitTarget:t=6,915.2`(float, 6915.2초 = 1시간 55분 지점)였는데, 파서의 정규식이 `,`에서 매칭을 멈춰 `6`만 읽었다. UE의 `float→string` 변환은 **천 단위 쉼표를 붙이는 로케일 포맷**을 타지만, 같은 로그 안의 `int→string`(예: `FXLAB:FXNOROW:63009999`, 8자리)은 **쉼표가 붙지 않는다** — 두 표기가 **같은 로그 파일에 공존**한다.

```
State:AwaitTarget:t=6,915.2      ← float, ★쉼표
FXLAB:FXNOROW:63009999           ← int, 8자리인데 쉼표 없음
```

**★발현 조건이 고약하다**: 값이 **1000을 넘어야만** 쉼표가 나타난다. 전투 시간이나 데미지 값이 세 자리 이내인 짧은 세션에서는 **영원히 재현되지 않는다** — *"지금까지 안 났으니 괜찮다"*가 근거가 되지 못한다. 실제로 `oracle_diff.rows_equal()`이 문자열 비교(`str(a)==str(b)`)였기 때문에, 라이브 `dmg=1,234`와 오라클 `1234`가 **불일치(가짜 FAIL)**로 잡히는 구조적 위험이 있었다 — 알파에서 HP/데미지가 네 자리에 도달하는 순간 조용히 터질 사안이었고, S1 봉인 판정이 이 비교기에 걸려 있었다.

**규칙**:
1. ★PIE 로그를 수치로 파싱할 때 **`[\d.]+` 같은 단순 정규식으로 쉼표를 놓치지 말 것** — 천 단위 구분자를 제거하는 정규화를 거친 뒤 숫자로 변환한다.
2. ★**숫자로 해석되는 값에서만** 쉼표를 제거하고, 비숫자 값(`A3`, `false`, 빈 문자열 등)은 원문을 보존한다 — "원문 보존"과 "숫자 정규화"의 책임을 분리할 것(RAW 계약과 비파괴적으로 공존).
3. 로그 비교/오라클 대조 로직(`rows_equal()` 류)이 있다면 **비교 직전에 이 정규화를 반드시 거치도록** 만들고, 회귀 테스트에 "정규화 함수 자체"뿐 아니라 "소비처가 실제로 그 정규화를 호출하는지"까지 포함할 것 — 헬퍼만 고치고 호출부를 안 고치는 실수가 실제로 나올 뻔했다.
4. "이상한 값"을 만나면 **결함으로 단정하기 전에 반드시 로그 원문을 끝까지 볼 것** — 이 프로젝트에서 같은 구조(측정/파싱 아티팩트를 실체로 오독)가 3fps 양자화 때도 있었다(함정95·100 계열).

**대응 구현**: `battle_log.parser.normalize_number()`(신설) — 숫자로 해석되는 값에서만 쉼표 제거. `oracle_diff.rows_equal()`이 비교 전 이걸 거치도록 수정. selftest 32/32 PASS(정규화 9건 + `rows_equal` 소비 확인 포함). 상세 계약: `docs/reference/전투로그.md` §1-6.

**출처**: 2026-08-12 전투완성 AU-A1-09([[AU-A1-09_T1전후_실측대조]]).

## 44. FT1-S2(SessionBoundary 마커 스캐폴드)에서 확정된 노하우 (2026-08-13)

> 배경: FT1-S2 — 라이브 `BP_BattleManager.EventGraph` 첫 수술. `SessionBoundary|` 마커 1줄 + 예약 `Sequence` 노드 심기([[FT1-S2_착수판정]] §3-2 화이트리스트). §43에 이어 **(109)부터.**

### 함정 (109) — `Sequence`(ExecutionSequence) 노드의 `create_node` 생성 문자열은 **카테고리와 노드명이 서로 다른 로캘로 섞여 있다** — 한글 카테고리 트리 전체를 뒤져도 안 나온다

**증상**: 같은 카테고리의 `RetriggerableDelay`가 앞선 S1 조회에서 한글 카테고리(`유틸리티|플로컨트롤|...`) 문자열로 실측됐기에, `Sequence`도 같은 패턴(`유틸리티|플로컨트롤|시퀀스`)일 것으로 추정하고 `find_node_types`를 그 한글 카테고리로 좁혀 탐색했으나 매칭 0건. 한글 트리 `유틸리티|플로컨트롤|` 아래에는 `Delay`/`RetriggerableDelay` 같은 latent류만 있고 `Sequence` 계열이 없다.

**원인**: `Sequence`의 실제 생성 문자열은 **카테고리는 영문(`Utilities|FlowControl|`), 노드명만 한글(`시퀀스`)** 인 혼합 로캘 문자열이었다. 함정(104)가 "에디터 로캘이 검색 문자열에 영향을 준다"를 이미 확정했지만, 이번 건은 그 확장판이다 — **같은 카테고리의 형제 노드끼리도 로캘이 갈릴 수 있고**, 한 노드의 생성 문자열 안에서조차 **경로 세그먼트마다 로캘이 다를 수 있다.** 카테고리 단위로 좁혀 탐색하면 이런 혼합 케이스를 구조적으로 놓친다.

**해법(실측)**: 좁힌 탐색이 0건으로 나온 시점에 **945건 전체 나열**(무필터 `find_node_types`)로 스캔해 한글 노드명 `시퀀스`를 찾아냈다 — 함정(104)가 이미 권고한 "0건이면 카테고리 전체 나열로 스캔"이 이번에도 유효한 공통 대응이었다.

**규칙**:
1. 같은 카테고리의 형제 노드(예: `RetriggerableDelay`)가 특정 로캘 패턴이었다고 해서 **같은 카테고리의 다른 노드도 같은 로캘**일 것이라고 가정하지 마라 — 노드 단위로 갈릴 수 있다.
2. 카테고리를 좁힌 탐색이 0건이면 **즉시 무필터 전체 나열로 전환**한다(함정104 공통 대응 재확인 — 이번엔 945건).
3. 확정된 생성 문자열은 발주서·화이트리스트에 **원문 그대로** 박아 다음 세션이 같은 탐색을 반복하지 않게 한다.

**출처**: 2026-08-13 스킬연출구조 FT1-S2 수술([[FT1-S2_착수판정]] §3-2 화이트리스트 N2).
