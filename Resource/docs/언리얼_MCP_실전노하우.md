...
**대응 구현**: `battle_log.parser.normalize_number()`(신설) — 숫자로 해석되는 값에서만 쉼표 제거. `oracle_diff.rows_equal()`이 비교 전 이걸 거치도록 수정. selftest 32/32 PASS(정규화 9건 + `rows_equal` 소비 확인 포함). 상세 계약: `docs/전투로그.md` §1-6.
