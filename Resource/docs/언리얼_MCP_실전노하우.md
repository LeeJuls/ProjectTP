...
### 함정 (82) — uasset 바이너리 grep은 "존재 증명"일 뿐 "최신 저장 증명"이 아니다 — 한계 명시 (정정, Codex FULL-003)
**배경**: BP는 `.gitignore`로 git 추적 밖에 있어, "이 작업이 실제로 디스크에 저장됐는가"를 git diff로 확인할 방법이 없어 보인다. 이 항목 원판은 uasset을 ASCII로 바이너리 grep하는 방법을 **"값싸게 검증하는 도구"**로 적었다. **Codex 전체 읽기전용 리뷰(FULL-003, P1)가 이를 정확히 지적했다**: *"uasset 문자열 존재와 파일 크기 유사성을 '저장 안 함' 근거로 쓰는 문서 지침이 실제 미저장 작업을 버릴 수 있음"*. Codex 말이 맞다 — 아래처럼 한계를 명시한다.
```powershell
...
**쓸 수 없는 경우 — 최신성 증명(불가능)**: 문자열이 **있다**는 결과는 "그 함수가 (그 시점 디스크본에) 존재한다"만 증명하지, **"그것이 최신 작업 내용대로 저장됐다"는 전혀 증명하지 못한다.** 함수 이름은 그대로 두고 내부 배선·로직만 바뀐 경우 grep으로는 신·구본을 **구분 불가능**하다. 파일 크기 유사성도 마찬가지로 정황일 뿐이다 — 실제로 다른 작업이 우연히 비슷한 크기가 될 수 있다. 이 용도로는 절대 단독 근거로 쓰지 말 것.
**더 확실한 방법**:
1. **dirty 상태를 직접 조회한다** — 함정⑳·함정(81)이 이미 쓰는 `is_dirty` 조회가 바이너리 grep보다 항상 우선한다. dirty=false면 저장 안 해도 안전하다는 직접 증거, dirty=true면 저장이 필요하다는 직접 증거다.
2. **직접 조회가 안 되거나 판단이 갈리면, grep 결과만으로 "저장 안 함"을 확정 짓지 말고 자동저장본부터 먼저 백업**해 되돌릴 수단을 확보한 뒤 판단한다. 2026-07-16 파트1 착수 전 실제로 이렇게 했다 — `D:\unreal\_savepoints\파트1_Start_착수전\autosave_참고\`에 자동저장본(`BP_BattleManager_Auto0_2133.uasset`·`BP_BattleSpawnPoint_Auto8_2046.uasset`)을 먼저 백업해 두고 나서 "저장 안 함"을 선택했다. grep 하나로 되돌릴 수 없는 선택을 한 게 아니라, 최악의 경우에도 자동저장본으로 복구 가능한 상태를 먼저 만들어 둔 것이었다.
**정직한 재평가**: 2026-07-16 F7b 저장 판단은 **결과적으로는 맞았다**(이후 세션까지 문제없이 이어짐으로 사후 확인됨). 하지만 그 판단은 grep 단독이 아니라 **정황 증거 3개의 조합**이었다 — ①F7b가 만든 함수 4개 전부가 디스크본에 존재 ②여러 자동저장본 크기가 서로 동일(=그 사이 실제 작업이 없었음을 시사) ③커밋 시각이 저장 시각보다 뒤(=작업 후 저장이 이미 있었다는 방증; 실측: 디스크본 2,389,751바이트 vs Autosave 2,389,823바이트, **72바이트 차**). 이 중 ③(커밋 타임라인)이 실제로는 가장 강한 근거였는데, 원판 제목은 이를 "바이너리 grep이 검증 도구"라는 단일 방법론으로 뭉뚱그려 잘못 대표했다. **다음에 같은 판단을 할 때는 grep을 단독 증거로 쓰지 말 것 — ①dirty 직접 조회 → ②(불가 시) 백업 확보 → ③정황 증거 복수 조합, 순서로 접근한다.**
**출처**: 2026-07-16 F7b 저장 판단(파트1 착수 직전 세션) · Codex 전체 읽기전용 리뷰 FULL-003([[Codex_전체_읽기전용_리뷰_2026-07-16]], P1) · 한계 명시 정정: 2026-07-17(Director).
