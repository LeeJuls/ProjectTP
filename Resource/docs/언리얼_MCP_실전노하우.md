...
### 함정 ㉓ `unreal-mcp`는 다중 에이전트의 동시 세션을 안정적으로 지원하지 않는다 — 3개 스파이크를 한 메시지로 병렬 launch하면 뒤늦게 접속한 세션이 "not connected"로 거부될 수 있다
알파 A0 스파이크 3건(UMG·합성 머티리얼·CSV 파이프라인)을 전부 unreal-mcp를 쓰는 에이전트로 동시에 launch했다. 그중 하나(CSV 파이프라인)가 절차 중간(`BlueprintTools.create` 직전)에 `MCP server "unreal-mcp" is not connected` 에러로 멈췄다. Director가 직접 진단: ①`Get-Process UnrealEditor`로 프로세스 확인 — PID 살아있고 `Responding: True`(멈춤·크래시 아님) ②`Get-NetTCPConnection -LocalPort 8000` — `Listen` 1개(정상 대기)와 함께 **`CloseWait` 2개**(최근에 닫힌 세션의 잔재) 확인 ③Director 자신의 `list_toolsets` 호출도 동일 에러로 실패, 재시도도 실패. 즉 **서버(에디터)는 건강하지만 신규/추가 클라이언트 접속이 거부되는 상태** — 동시에 실행 중이던 다른 두 스파이크 에이전트가 세션을 점유하고 있었을 가능성이 유력(엔진 로그에 CSV 스파이크가 호출하지 않은 툴 호출들이 같은 시간대에 찍혀 있었다는 것이 방증).

**실전 절차**: 여러 에이전트가 **같은 UE 에디터(unreal-mcp)를 동시에 건드리는 작업**은 한 메시지로 병렬 launch하지 말 것. 대신 ①MCP를 안 쓰는 순수 조사·문서 작업은 병렬 무방(예: 지금까지의 designer 논의, art-pipeline의 Pillow 실측 등) ②MCP를 쓰는 작업끼리는 **직렬로 하나씩**, 또는 병렬로 띄우되 접속 실패를 **정상적인 재시도 대상**으로 취급(짧은 대기 후 1~2회 재시도, 그래도 안 되면 다른 동시 작업이 끝나길 기다렸다가 재개 — §9 원칙 "3회 실패 시 중단"과는 별개로, 이건 로직 실패가 아니라 인프라 경합이므로 무한 재시도가 아니라 "동시 작업 종료 대기 후 1회 재개"가 적절한 대응). 이 사례에서 CSV 스파이크 에이전트는 정직하게 실패를 보고하고(허위 성공 보고 없이) 정확한 재개 지점을 문서에 남겼다 — 이게 올바른 대응이었다.
**교훈**: Director가 "독립적인 일은 한 메시지에서 여러 에이전트 병렬 호출"이라는 일반 원칙을 적용할 때, **"독립적"의 기준에 "같은 외부 리소스(unreal-mcp 세션)를 공유하지 않는다"를 추가**해야 한다. 순수 로직상 독립이어도 같은 MCP 서버에 동시 접속하면 인프라 레벨에서 충돌한다.
