...
- **▶ ★기획 방향성 2 공식 확정(2026-07-11)** — Steam 드래프트 단판 PvP([[기획_방향성]]): 매판 1000캐릭 중 6개→5픽→1밴(교차밴)→4:4→광폭화, 자산 없음(골드만 이월). 방향성 1(모바일 수집형)은 `docs/방향성1_백업/` 보관. **로드맵·알파개발계획 방향성2 재작성 완료(2026-07-13)** — 이때 등급축 유지+배율압축 확정, 데이터·서버 규약을 [[데이터_서버_규약]]으로 분리. 다음: 알파 단계별 상세화(A1 전투완성·A2 캐릭터코어부터).
