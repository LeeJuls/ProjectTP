...
- **✅ 기본전투무대(4v4) 완료** — Village Pack 배경 위 heroes99 8종 대치 + idle 애니 + Lit 라이팅. **스프라이트 z-fight 근본해결**. (`docs/features/기본전투무대/`)
- **✅ 팀·프로세스·S0 아트검증** — Director+9([[에이전트팀_설계]]) + 게이트 워크플로우([[개발_워크플로우]]) + Fable=두뇌 원칙.
- **▶ 다음** — 풀테스트(F: qa+verifier+방향부합) 후 전투 시스템 설계 또는 다음 기능.
