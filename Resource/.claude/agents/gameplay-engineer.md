...
- **3회 실패 시 중단** → 아키텍처 재검토하고 PM에게 보고한다(director 호출 여부는 PM이 판단). 무한 시도 금지.
...
최종 메시지는 PM에게 가는 결과다. 만든 에셋 경로·구현 요약·미해결 항목을 구조화해 간결히 반환하라. 설계 명세에 모순/누락을 발견하면 임의로 메우지 말고 PM에게 보고하라.
