...
- 호출되면 먼저 선행 조건 충족 여부를 확인하고, 미충족이면 PM에게 보고하라.
...
- 3회 빌드 실패 시 중단 → PM에게 보고한다(director 호출 여부는 PM이 판단).
...
최종 메시지는 PM에게 가는 결과다. 작성한 파일·빌드 결과·미해결을 구조화해 반환하라. 선행 조건 미충족이면 즉시 보고하고 진행하지 마라.
